...
## [2026-07-15] — Session 63 (continued): `/api/health` hardened for external uptime monitoring

**Context:** User asked what else was worth doing now that the site is live to real strangers (Session 61/62). Beyond the SEO work above, flagged that the site has error monitoring (Sentry, since Session 61) but no uptime monitoring — `/api/health` exists but nothing external polls it, so an outage would currently surface via a user complaint rather than a page. The user separately confirmed Google Search Console verification + indexing request were already done (2026-07-15/16) — `TASKS.md`'s Growth section updated to reflect that (next actionable step is checking the Performance report for real impression data once it lands, days-to-weeks out).

User asked to prep `/api/health` for an external monitor. The endpoint (`src/app/api/health/route.ts`) was already reasonable — `force-dynamic`, checks DB/Auth/Realtime separately, correct 200/503 status codes — but had two real weaknesses as a monitoring target specifically:
- **The three checks ran sequentially.** Each `await`ed in turn, so worst-case latency stacked (DB + auth's 5s timeout + realtime's 5s timeout), risking a monitor's own timeout tripping during a partial outage and reporting "down" when the app was merely slow.
- **The database check had no timeout at all** (unlike auth/realtime, which already used `AbortSignal.timeout(5000)`) — a network stall against Supabase could hang the route indefinitely.

**Fix:** Extracted each check into its own function returning a plain status string, ran all three concurrently via `Promise.all` (was sequential awaits sharing a mutable `checks`/`allOk` object), and added a `.abortSignal(AbortSignal.timeout(5000))` to the database query to match the other two checks' existing timeout pattern. Also added an explicit `Cache-Control: no-store` header so no CDN/proxy layer can ever serve a monitor a stale cached "ok". Response JSON shape, field names, and status-code semantics are byte-for-byte unchanged (`{status, database, auth, realtime, timestamp}`, 200 when all reachable, 503 otherwise) — this was a reliability/latency fix, not a contract change.

**Verified live**, not just via typecheck: ran a real `next build` + `next start` on a scratch port with real `.env.local` Supabase credentials, `curl`'d `/api/health` and got `{"status":"ok","database":"reachable","auth":"reachable","realtime":"reachable",...}` with `Cache-Control: no-store` present; timed 3 consecutive requests at ~2.1–2.2s each, consistent with the three checks now bottlenecked by the single slowest network round-trip rather than stacking sequentially (previously could have summed toward 3x that under a slow/degraded backend). `npm run verify` clean (0 errors, same 4 pre-existing unrelated warnings).

**Note for the user:** creating the actual external monitor (UptimeRobot, Better Uptime, etc.) requires a third-party account signup — that step wasn't done here, only the endpoint itself was hardened to be a good target for one.

**Follow-up (same day) — user set up UptimeRobot** (free tier, their own account), monitoring `spintra.io` every 5 minutes — confirmed working via a screenshot (100% uptime, 1/50 monitors used). Flagged one thing worth the user double-checking themselves: the dashboard's truncated display showed `spintra.io/` rather than the full `/api/health` path, and monitoring bare `/` instead would miss a Supabase-only outage (the homepage is mostly static and can return 200 even when the DB/auth/realtime backend is down — exactly the failure mode `/api/health` exists to catch). User confirmed they were satisfied with the current setup; not pursued further. Saved as a reference memory (`reference_uptime_monitoring.md` in the assistant's cross-session memory, not this repo) so a future session knows monitoring exists and what to verify if it's ever revisited.

---

## [2026-07-16] — Session 63 (continued): Per-tool OpenGraph images

**Context:** Continuing the Session 63 growth work, the next item from `TASKS.md`'s Growth backlog was per-tool social-share images — all 14 `/tools/*` pages shared one generic `/og-image.png`, so any link shared to a specific tool (Discord, group chat, Reddit — this already happens organically via word-of-mouth, independent of any deliberate marketing push) showed a generic Spintra banner regardless of which tool it was. The user explicitly asked for extra rigor on this task (responsive/proactive/responsible/reliable/accountable), given here specifically because OG-image generation via Satori (the renderer behind Next's `next/og` `ImageResponse`) is a case where code can typecheck and build successfully while still silently rendering wrong (garbled fonts, invalid colors, text overflow) — failures a build never catches.

**Two things were verified empirically instead of assumed, both of which would have been wrong if guessed from memory:**
- **Tailwind's resolved gradient colors.** `GAMES[].color` values (e.g. `"from-cyan-500 to-blue-500"`) are plain Tailwind utility classes, but Satori doesn't parse the `oklch()`/`lab()` color functions Tailwind v4's default palette actually compiles to (confirmed by grepping `globals.css`'s compiled output) — a hardcoded "standard Tailwind hex palette" table from memory would in several cases have been visually wrong, since Tailwind v4's default color scale values differ from v3's memorized hex approximations (e.g. this project's real `cyan-500` resolves to `#00b8db`, not v3's commonly-cited `#06b6d4`). Ground truth was extracted by running the actual built app, using Playwright to read `getComputedStyle()` on the real `/tools` grid's gradient icons for all 14 tools, then converting the reported `lab()` triples to sRGB hex with the `culori` library (installed only in a scratch directory outside the repo, never added as a project dependency) rather than hand-rolling Lab→sRGB conversion matrices from memory. Result: `TOOL_GRADIENT_HEX` in the new `src/lib/og-image.tsx`.
- **Whether Next's built-in OG image renderer needs a custom font.** Rather than assume `next/og`'s `ImageResponse` needs a font passed in (true for the standalone `@vercel/og` package on non-Next platforms), an actual test image was generated and visually inspected (via `Read` on the fetched PNG) before rolling out to all 14 routes — confirmed Next's bundled default font renders cleanly with no tofu-box/garbling risk, so no font-loading dependency (and its associated fetch-at-generation-time fragility) was needed.

**Delivered:**
- **New `src/lib/og-image.tsx`** — single source of truth: the verified `TOOL_GRADIENT_HEX` lookup plus `renderToolOgImage(href)`, which builds a Satori-compatible JSX tree (title, tagline, "Free · No sign-up · Play instantly" badge, small Spintra wordmark) reading `label`/`desc` from the existing `GAMES` registry. Deliberately restricted to Satori's reliably-supported CSS subset (flexbox, solid colors, `linear-gradient`, `borderRadius`) — no blur filters or nested SVG icons, both less certain to render correctly, avoided for reliability over marginal visual polish.
- **14 new `src/app/tools/*/opengraph-image.tsx` files** — Next.js's file-convention special route (`next/og`'s `ImageResponse`, `size = {width:1200,height:630}`, `contentType = "image/png"`), each just re-exporting the shared render function with its own href. Next.js auto-generates the `<meta property="og:image">` tag per route from this convention — no manual edit to `tool-metadata.ts` needed.
- **Zero existing files modified** — fully additive; any route without its own `opengraph-image.tsx` keeps inheriting the original shared `/og-image.png`.

**Verified, not just typechecked:**
- `npm run verify` clean (0 errors, same 4 pre-existing unrelated warnings) both after the first test route and after all 14.
- Real `next build` — all 14 `/tools/*/opengraph-image` routes appear and prerender static (`○`), no build errors.
- **Visually inspected actual generated PNGs** (not just confirmed HTTP 200) for 3 deliberately diverse cases: Lucky Wheel (cyan/blue, the first end-to-end test), Rock Paper Scissors (the single longest tool label, checking for text overflow — fits comfortably with room to spare), and Trivia (the brightest/highest-risk gradient for white-text contrast — confirmed legible, consistent with the same gradient already carrying white icon text elsewhere in the live app). All three: correct gradient, correct text, no font/rendering artifacts.
- **Confirmed the actual behavioral effect, not just that the image route works standalone** — fetched a real running `/tools/trivia` page and confirmed its `<meta property="og:image">` tag now reads `https://spintra.io/tools/trivia/opengraph-image?<hash>`, replacing the previously-inherited shared `/og-image.png`.
- `culori` (used only for the one-time hex-conversion script) was installed in an isolated scratch directory, never touched `package.json`/`package-lock.json` — confirmed via `git status` showing only the 15 intended new files with zero modifications to any existing file.

---

## [2026-07-16] — Session 63 (continued): Critical — `rooms` table privacy bypass (migration 0062)

**Context:** The user asked whether the Explore page's "Live Trending Rooms"/"Recent Activity" empty states were genuinely wired up. Investigating that (see the preceding entry) surfaced a real, currently-exploitable security finding one level deeper: the user then suggested showing locked/private rooms on Explore for a "social proof" effect, which prompted checking exactly what a private room's "Off = invite-only via code" promise (`create-client.tsx`) actually enforces at the database level. It didn't enforce anything.

**Finding:** `rooms_select` (`0005_enable_anonymous_auth_rls.sql:22`) has been `using (true)` since anonymous auth was introduced — any row, public or private, was selectable by anyone. Migration `0009` hardened `room_participants` and `chat_messages` to require room membership or a public room, but the `rooms` table itself was never touched, and migration `0035`'s own comment (`"Before: rooms has for select using (true) — any authenticated user can..."`) explicitly acknowledged this as a known, still-true fact at the time and left it unaddressed. Confirmed live against production with the plain public anon key (no privilege escalation — this is exactly what the app's own browser client can do): a direct REST call returned all 294 rooms including private ones, with each row's `code` — the actual join credential — fully exposed. A stranger did not need an invite to join any private, unlocked room; they could enumerate its code directly.

**Why the fix isn't a one-line policy tweak:** RLS decides row visibility from the requester's identity alone; it has no way to see *how* a query was filtered. A policy tight enough to block "list every private room" would equally block "look up the one room I have a real code for" — both are, from the database's point of view, just "a SELECT against this table." The two cases can only be told apart by using a different mechanism for the legitimate one.

**Fix — `supabase/migrations/0062_rooms_select_privacy_fix.sql`:**
1. Tightened `rooms_select` to `is_public = true OR host_id = auth.uid()::text OR is_member_of_room(code, auth.uid()::text)` — the identical pattern `0009` already established for the other two tables.
2. Added `get_room_by_code(p_code text)`, a `security definer` RPC (matching the existing `is_member_of_room`/`elect_room_host` house style): takes exactly one code, returns at most one row (`code` is `unique`), so enumeration is structurally impossible through it regardless of caller identity.
3. **New `src/lib/room-lookup.ts`** (`getRoomByCode` helper) and **6 client call sites converted** from a raw `.from("rooms").select()` to this RPC — every place that looks up a room by code *before* the caller is necessarily a member yet: `room-join-check.ts`'s pre-join validation, `room-client.tsx`'s `verifyAccess`, `create-client.tsx`'s code-collision check (generating a new room), and three call sites in `use-room-subscription.ts` (`trackSelf`'s fallback fetch, `loadRoomDetails`'s fallback fetch, and the kicked-vs-closed distinguishing check — this last one specifically needs to see a private room's row *after* the caller's own participant row was just deleted by a kick, which is exactly the "no longer a member, but still legitimately need this one row" case the RPC exists for).
4. **Left unconverted, deliberately:** `room-settings-panel.tsx` (renders only for `isHost`, always post-membership), the explore page's public-room browse query (already `.eq("is_public", true)`, a genuine list/browse operation the RLS now independently backs up), and `/api/health`'s reachability check (never signs in, only checks for a query *error*, not row content — confirmed unaffected by reasoning through the auth.uid()-is-null case, since RLS restricting visible rows isn't a query error).
5. **`database.types.ts`** manually updated with the new RPC's generated shape (regenerated via `supabase gen types --local` to get the exact shape, then hand-merged into the existing file rather than overwriting it wholesale, since the fresh regeneration had incidental unrelated diffs from what was likely a different original CLI version — a stray log line and a dropped `__InternalSupabase` metadata block neither related to this change).

**Verified against a local Docker Supabase reset — before touching production at all:**
- Reproduced the exact vulnerability locally first: seeded a known private test room, confirmed the anon key could read it before the fix.
- Applied migration `0062` locally (`supabase db reset`, clean apply, no errors), re-seeded, and confirmed: (a) broad enumeration now returns empty, including when using a **freshly-minted real anonymous session** (not just a bare unauthenticated API-key request — ruling out "just sign in anonymously" as a trivial bypass); (b) `get_room_by_code` still correctly returns the room for an exact code, confirming the legitimate join flow keeps working; (c) after actually inserting a `room_participants` row for a test user, that same user's normal `SELECT` against `rooms` correctly resumed working — confirming the already-a-member call sites left unconverted don't regress.
- **Full Playwright suite run against the local instance with the app's real `.env.local` temporarily repointed at it (backed up first, diff-verified identical after restoring)**: 63/63 passed, including the exact scenarios this change touches — create+join, two participants joining a room together (private by default, since `isPublic` defaults `false`), reconnection, host-kick (exercises the exact call site converted for the kicked-vs-closed check), and host-edits-settings-live-sync-to-guest (confirms the deliberately-unconverted `room-settings-panel.tsx` path still works).
- `npm run verify` and `npm run build` clean (0 errors; same 4 pre-existing unrelated warnings) after restoring production `.env.local`.
- `docs/ARCHITECTURE.md` §4's migrations table updated (this project's `docs:check` script enforces every migration has an entry there — caught the omission on the first `npm run verify` pass after writing the migration).

**Not yet applied to production** — migration `0062` exists locally and is verified against a local reset, but has not been run against the live Supabase project. Given this changes a live RLS policy touching every one of 294 real rooms, this is being held for the user's explicit go-ahead before applying, same as every other production-facing change this session.

**Not yet committed/deployed** — awaiting the user's go-ahead, per this session's established pattern of confirming before each push to `main`.

---

## [2026-07-17] — Session 64: `/for-teachers` landing page (organic-traffic growth)

**Context:** Following on from Session 63's growth work, the user asked to make the site "go viral" / get noticed. Grounded that in what's actually buildable: code changes don't create virality by themselves — the last open item in `TASKS.md`'s Growth backlog, a dedicated `/for-teachers` landing page, was the next concrete lever (teachers are a high-intent search audience — "random name picker classroom", "team generator for class" — who also organically reshare tools in teacher Facebook groups/Pinterest/Reddit). The user also asked to check Google Analytics for real traffic/conversion data; there is no connected tool for GA4 in this environment (only Gmail/Calendar/Drive are wired up), so that half of the ask was explicitly declined rather than guessed at, and left to the user to check directly.

**Compliance check before writing content:** `src/app/legal/privacy/page.tsx` §6 states the Service is "not directed at children under 13" (standard COPPA-avoidance boilerplate). A page marketing directly at K-12 teachers for classroom use could sit in tension with that if it reads as "hand this to your under-13 students." Resolved by framing the whole page around the **teacher as operator** (runs it on the classroom display; students never sign up — which was already true of the product before this page existed) and making zero new compliance claims (no "COPPA-safe"/"FERPA-safe" language anywhere) — every factual claim on the page (no email/password/real name required, 6-character private room code, lockable rooms, 50-participant cap) is one already backed by the existing Privacy Policy or `ARCHITECTURE.md`'s documented capacity ceiling, not a new promise.

**Delivered:**
- **`src/app/for-teachers/page.tsx`** — new static server component (no client JS): hero with two CTAs (`/create?type=classroom`, an in-page anchor to the tools grid), a trust-point row (free, no student sign-ups, Chromebook-compatible, 50-student room cap), a curated 6-tool "Ideas for your classroom" section pairing each tool with a specific classroom use, the full `GAMES.filter(g => g.classroomSafe)` grid (11 tools — reuses the existing registry, no new data duplicated), an FAQ section with `FAQPage` JSON-LD (same pattern as `tool-seo-section.tsx`), and a closing CTA. `alternates.canonical` + OpenGraph metadata exported directly from the page (no separate layout.tsx needed, unlike the tool pages, since there's no client-component widget to wrap).
- **`src/app/for-teachers/opengraph-image.tsx`** — new share-card image. Rather than transcribe a fresh, unverified `sky-500`/`cyan-500` hex pair (the exact mistake Session 63's OG-image work explicitly avoided for the 14 tool cards — Tailwind v4's `oklch()`-based defaults aren't safe to hand-guess), reused Bingo's already Playwright-verified gradient (`#00bba7` → `#0092b8`) from the existing `TOOL_GRADIENT_HEX` table.
- **`src/lib/og-image.tsx` refactored, non-breaking:** extracted the JSX-building logic into a new exported `renderOgImage({title, desc, gradient, tagline})`; `renderToolOgImage(href)` now just resolves a tool's title/desc/gradient from `GAMES`/`TOOL_GRADIENT_HEX` and calls it. Output is byte-for-byte identical for all 14 existing tool OG routes — this only removes duplication for the one new non-tool caller.
- **`src/app/sitemap.ts`** — added `/for-teachers` to `staticPaths` (the same gap Session 63 found and fixed for bare `/tools` — a new route is invisible to crawlers unless it's in this list, `robots.ts`'s allow-by-default rule alone isn't enough for discovery).
- **`src/app/page.tsx`** — added a "For Teachers" link to the homepage footer, next to the existing Explore/Create Room/Terms/Privacy links.

**Verified, not just typechecked:**
- `npm run verify` clean (0 errors; same 4 pre-existing unrelated warnings in `use-room-subscription.ts`/`room-client.tsx`, untouched by this change). One real lint error caught and fixed along the way: an unescaped apostrophe in the hero copy (`react/no-unescaped-entities`).
- `npm run build` — `/for-teachers` and `/for-teachers/opengraph-image` both prerender static (`○`), alongside all 14 pre-existing tool routes and their OG images, confirming the shared `og-image.tsx` refactor didn't break anything.
- **Real browser verification**, not just a build check: started the dev server, drove it with Playwright (Chromium) rather than `next/font`-guessing — screenshotted the page in both light and dark color schemes (top/mid/bottom of the page), clicked a FAQ `<details>` to confirm the accordion actually opens, and fetched `/for-teachers/opengraph-image` directly to confirm it serves a real `200 image/png` and visually inspected the rendered PNG (title, tagline, gradient all correct, no font/overflow artifacts). Zero `console.error`/`pageerror` events captured across both theme passes. Killed the dev server and removed the scratch Playwright script afterward.
...
## [2026-07-17] — Session 64 (continued): activity-picker card heights were uneven, plus a verification-process note

**Context:** Immediately after the icon-badge fix above, the user shared another screenshot: with the flat gray icons replaced by colored badges, a second, previously-less-visible layout problem became obvious.

**Finding:** `activity-picker-dialog.tsx`'s card buttons had no fixed or minimum height — each button's height was purely a function of its own label's wrapped line count. "Rock Paper Scissors" wraps to 3 lines and stood visibly taller than "Trivia" or "Guess Number" (1-2 lines) in the same grid row, giving the grid a jagged, uneven appearance. This was pre-existing (not introduced by the icon-badge fix), just harder to notice with plain 24px gray glyphs than with the now-larger colored badges drawing the eye to the grid.

**Fix:** added `min-h-36 justify-center text-center` to the card button's className. Every card is now the same height regardless of label length, with its icon badge and label vertically centered inside.

**Verified:** real local dev-server run, single Playwright script (create a classroom room → open the dialog → screenshot), not a loop — confirmed every row now renders with uniform card heights. `npm run verify` clean (0 errors, same 4 pre-existing unrelated warnings).

**Process note, logged for accountability:** the *previous* production-verification step in this session (confirming the icon-badge fix was live on spintra.io) used a `until ... grep -q ...; do sleep 10; done` poll loop wrapping a Playwright script with a broken CSS selector (`sm:grid-cols-3` used as a class selector without escaping the colon). The script threw on every attempt, so the loop kept retrying every 10 seconds — and because the failure happened *after* the script's "create a room" step, each retry created a real room on the production database before crashing. This ran unnoticed across several conversational turns until the user asked "done?" a third time, prompting a check of the task's raw output rather than continuing to assume it was silently working. Stopped the loop immediately on discovery. **No manual database cleanup was performed** — the rooms created this way are private (`is_public: false`, the create flow's default), never listed on Explore, and no longer enumerable via the anon key (Session 63's RLS fix). They fall under the existing `cleanup_inactive_rooms()` `pg_cron` job (removes rooms with zero online participants once 2+ hours old, documented in `ARCHITECTURE.md` §10) the same as any other abandoned room — no new mechanism was needed or added. The corrected verification approach for the rest of this session: run a check script once, standalone, and read its actual output before ever wrapping it in a retry loop against a production system.

---

## [2026-07-17] — Session 64 (continued): shared Dialog component had no height/scroll constraint

**Context:** Immediately after the card-height fix above, the user shared a screenshot showing the same dialog cropped at both the top and bottom on a short viewport, with the cut-off content unreachable — no scrollbar, no way to see it.

**Finding — traced up one level instead of patched locally:** `src/components/ui/dialog.tsx`'s `DialogContent` (the shared primitive underlying all 11 dialogs in the app — Join Room, Close Room, Room Settings, Moderation Dashboard, the activity picker, etc.) constrained horizontal size to the viewport (`max-w-[calc(100%-2rem)]`) but had no equivalent vertical constraint and no `overflow-y-auto`. Positioned via `fixed top-1/2 left-1/2 ... -translate-x-1/2 -translate-y-1/2`, any dialog whose content exceeds viewport height simply overflows equally past both the top and bottom edges with nothing to scroll — a structural gap that predates this session entirely. The immediately preceding fix (uniform `min-h-36` cards) made this specific dialog's total content taller and therefore more likely to hit the ceiling, but the underlying bug wasn't specific to that dialog or that fix.

**Fix:** added `max-h-[calc(100%-2rem)] overflow-y-auto` to `DialogContent`'s base className, directly mirroring the existing width-constraint pattern (same `calc(100%-2rem)` value, same 1rem-per-side viewport margin). Purely additive CSS — dialogs whose content already fits the viewport are unaffected; only content taller than the viewport now scrolls instead of clipping.

**Verified with actual QA rigor this time, not just a re-check of the original complaint:**
- Reproduced the exact reported scenario (1920×760 viewport) against the activity picker — confirmed via the dialog's real `getBoundingClientRect()` that it now sits fully within the viewport (`y: 16`, fits).
- Deliberately went harsher: a 1000×500 stress-test viewport (well below any expected real device) — dialog still fits at the same 16px margin, with the remaining cards scrollable into view rather than vanishing. Screenshot visually confirms.
- **Regression check on an unrelated dialog** (the navbar's Join Room dialog, a heavily custom-styled consumer of the same shared component) at a normal viewport (1400×900) — confirmed pixel-identical rendering to its pre-fix appearance, no unwanted scrollbar, nothing broken by the shared-component change.
- `npm run verify` clean (0 errors, same 4 pre-existing unrelated warnings).

**Process note:** this whole chain (icon badges → uneven heights → viewport overflow) is a direct example of why "fix exactly what was pointed at and stop" isn't sufficient QA — each fix's side effect surfaced the next bug. The height/scroll fix here was deliberately done at the shared component level specifically so it wouldn't need repeating for the other 10 dialogs whenever one of them eventually grows enough content to hit the same ceiling.

---

## [2026-08-29] — Session 65: "Spintra City" — design, research, and review only (zero code)

**AI:** Claude Code (Opus 5 / Sonnet 5)
**Task:** Turn a design conversation the user had with a *different* AI outside this repo into a durable handoff doc, then review it, verify its research, and plan its integration into the existing site.
**Files Modified:** `docs/SPINTRA_CITY_DESIGN.md` (NEW), `docs/SPINTRA_CITY_CONTENT.md` (NEW), `docs/INDEX.md`, `docs/TASKS.md`, `docs/AI_CONTEXT.md`, `docs/HANDOFF.md`
**No source files, migrations, or assets were touched. Nothing is built.**

**Purpose:** The user was switching AI tools mid-design and needed the state captured so another model could continue; then, across follow-up requests, asked for a design review, deeper research, and a plan for how the feature would actually connect to the existing site.

**Outcome, in the order the work happened:**

1. **Handoff doc created.** Transcribed the design conversation with deliberate provenance separation — what the user *actually approved* (human-first/no bots, 2–8 players; a server-authoritative PostgreSQL match engine that explicitly rejects the existing 14 games' client-driven `activity_state` event-log pattern; a 5-table `city_*` data model) kept distinct from what was only *proposed and never confirmed* (the turn-flow/reliability design incl. a 90s turn clock — the conversation ended mid-review on that exact question when the user switched tools). This separation was the point of the exercise: nothing gets silently promoted from "proposed" to "decided" in a handoff.

2. **Design review** against this repo's own documented bug history — 12 findings, 2 Critical: (a) no identity-binding for match commands, in an app whose entire identity model is anonymous and trivially rotatable (fine for RPS, not for an in-match economy); (b) high risk of repeating this repo's single most-repeated bug class — a secret exposed via an over-permissive RLS `select`, which has already shipped and been fixed three times here (trivia answer key `0045`, guess-number secret `0028`/`0057`, Bingo verification `0048`) plus `rooms_select` itself (`0062`) — since `city_matches` is designed to store a seeded deck order.

3. **Verified the richup.io research** the original conversation had partly reasoned from, which had never been independently checked. **Found a real factual error:** the claim that richup has no bot feature is false — it does. Also found richup's actual shipped turn timer is a chess-clock model (free thinking time, then a carried-over personal reserve), its answer to unresponsive players is elimination after exactly 2 auto-played turns, and a genuine user-reported reconnect-to-spectator bug worth explicitly testing against. The erroneous claim had been load-bearing in the original no-bots reasoning; the decision still stands on its own merits, but no longer cites richup as precedent.

4. **Two parallel deep-research passes.** One on genre-wide player-facing UX (richup.io via a read-proxy that got past its Cloudflare 403, plus Rento Fortune as a second comparable product). One producing a **file-level integration plan by reading this repo's actual current code** rather than trusting `ARCHITECTURE.md`'s prose. That pass found: the clean seam (`room-client.tsx`'s `RoomGameArea` *already* special-cases `party`/`classroom`, so a third `"city"` branch rendering a dedicated `CityMatchShell` follows an established pattern rather than inventing one); a real gotcha (`rooms.type` carries a DB CHECK constraint from migration `0039`, so adding `"city"` to the TypeScript union alone would fail at the DB layer — the migration must land first); and the exact existing conventions City's RPCs should copy (`auth.uid()`-derived identity per `award_score`, `pg_advisory_xact_lock` per `0029`, `*_attempts` rate-limit triggers per `0011`/`0025`/`0030`/`0033`/`0038`) — which resolved three of the review findings in direction, including the Critical identity one.

5. **7 open product questions decided by explicit user delegation** ("take decision based on whatever is best suited"), each tagged **DECIDED (by delegation)** in the doc to stay distinct from the user's own approvals: chess-clock turn timer over a flat 90s; 2-autopiloted-turn forfeit ceiling; mid-match kick routes through the retire/liquidation sequence; City rooms get a longer cleanup threshold than the standard 2h `cleanup_inactive_rooms()` rule; City rooms do list publicly on `/explore`; full mobile parity (not richup's desktop-first hedge); end-of-match recap layers a net-worth summary on top of the existing Scoreboard/XP system rather than replacing it.

6. **Board content drafted.** User picked "The Wheelworks" (a city powered by giant fortune wheels, currency "Spins") from three original concept pitches, and confirmed the classic 40-space structure should be retained with original names/art only. `docs/SPINTRA_CITY_CONTENT.md` now holds the full 40-space layout, per-group economy/rent tables, both 16-card decks, 8 tokens, and rule toggles — including one deliberately-off-by-default Spintra flourish (a Bonus Spin on the free-rest corner) so the classic core stays intact.

**Unrelated pre-existing bug found along the way (NOT fixed, flagged only):** `ARCHITECTURE.md` documents `.glass`/`.glass-card` Tailwind utility classes that **do not exist** in the codebase. The real pattern is CSS custom properties (`--surface-glass`, `--surface-panel`) consumed via Tailwind v4 arbitrary-value syntax, e.g. `bg-(--surface-glass-strong)`. Worth a separate doc fix; recorded here so it isn't lost.

**Risks:**
- **Nothing has been validated by execution** — this is design and research only. No migration, component, or art exists; no `npm run verify` run was relevant to this session since no source changed.
- **The content draft's economy has never been playtested.** Prices, rents, salary, and starting cash are a reasoned first draft explicitly derived rather than copied (see the content doc's IP note) — they need a real balance pass before launch.
- **Several design questions remain genuinely open** (turn state-machine step detail, reconnect grace period, late-arrival spectator rule, net-worth mortgage formula, auction interaction detail, bankruptcy sequence, trade-offer staleness, `city_match_players` denormalization risk, all board art). `SPINTRA_CITY_DESIGN.md` §7 and §8 track them.
- **Scope risk is the headline concern.** This is by far the largest feature ever attempted in this repo — the other 14 games are small self-contained activities; this is a stateful economy with ~5 tables, a dozen RPCs, and board/trade/auction UI. §8 mitigates by planning 7 independently-verifiable vertical slices, with Slice 1 (room type + lobby + seats, no gameplay) as the architectural proof that must not be skipped.

---

## [2026-09-03] — Session 66: Spintra City launch-readiness check, full e2e run, stale test-port fix, docs sync
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User asked "is our game ready to publish?" then, after a gap analysis, asked to proceed with the non-destructive parts (full CI/e2e verification + docs sync), holding the migration push and merge for explicit go-ahead.
**Files Modified:** `docs/SPINTRA_CITY_SPEC.md`, `docs/SPINTRA_CITY_DESIGN.md`, `docs/TASKS.md`, `docs/AI_CONTEXT.md`, `docs/HANDOFF.md`, `docs/CHANGELOG_AI.md` (this entry); `tests/qa-city-helpers.ts` + 11 `tests/qa-x*.spec.ts` files (stale port fix — see Outcome).

**Purpose:** Between Session 65 (design-only handoff) and now, 49 commits had actually implemented the entire Spintra City feature on local branch `feat/spintra-city-design` — all 7 slices, an 8-round/44-bug QA hardening pass, and a visual/UX review — none of which was ever reflected back into `AI_CONTEXT.md`/`HANDOFF.md`/`TASKS.md`/`SPINTRA_CITY_SPEC.md`, all of which still read "design phase, zero code." This session's job was to establish ground truth (not trust the stale docs), verify what actually works, and correct the record.

**Outcome:**
- **Confirmed via git**: the branch has 49 commits ahead of the point it diverged from `main`, has no remote/upstream (never pushed), and `main`/`origin/main` are fully in sync with each other and unaware of this work.
- **`npm run verify`** (typecheck+lint+docs:check) and **`npm run build`** — both clean on the branch.
- **`npm run test:city-regression`** (a 57-case harness mapped 1:1 to the QA audit's bug IDs, calling RPCs directly against local Docker Supabase) — **57/57 passing.**
- **`npm run verify:migration` against the live production Supabase project** — confirms migrations `0063`–`0091` (the entire city chain) are **not applied live**; `city_settle_auction` does not exist in production. Applied and verified on local Docker only. This repo has hit this exact "tracked as applied but never ran" failure mode three times before (`0008`/`0009`/`0010`) — flagged so it isn't repeated a fourth time for city.
- **`npm run ci`** stops at the `npm audit --audit-level=high` gate (14 vulnerabilities, 10 high — Next.js/PostCSS/sharp/browserslist/etc.). Confirmed via `git diff --stat main -- package-lock.json` (only 1 line changed, in `package.json`, adding the new `test:city-regression` script) that this is **pre-existing on `main`**, not caused by this feature — a separate, repo-wide dependency-freshness gap.
- **Full `npm run test:smoke` run #1 (before any fix): 61/93 passed.** Investigated the failures rather than assuming either "real regressions" or "flake": found `playwright.config.ts`'s `webServer` runs the app on **port 4000**, but 12 test files (`tests/qa-city-helpers.ts` — the shared `BASE` constant used by `qa-x12` through `qa-x19` — plus 11 more `qa-x*.spec.ts` files with their own local `BASE` constant) hardcoded `http://127.0.0.1:4020`, a port nothing was listening on (confirmed via `netstat`). These are clearly QA-audit scratch tests originally run standalone against a manually-started dev server on a different port, never reconciled with the repo's actual `playwright.config.ts` convention. Root cause was confirmed, not assumed: `docker exec supabase_db_Spintra-1 psql -c "select 1"` (the DB-side helper these same tests use) succeeded in 0.176s, ruling out a database/Docker explanation; the actual browser error observed mid-run (`TypeError: Failed to fetch (qjxaehxwuqntyqrdmihs.supabase.co)`) and an explicit `net::ERR_CONNECTION_REFUSED at http://127.0.0.1:4020` pinned it to the port mismatch specifically. The 12 broken tests each burning up to 4 minutes retrying a dead connection, 8 of them in parallel, appears to have also caused **collateral failures in unrelated, previously-verified tests** sharing the same machine/worker pool during the run (room settings live-sync, trivia scoreboard, bingo 15-call stress test, moderation dashboard) — all timed out in the 1–1.5 minute range, a pattern absent from this repo's prior "verified live" runs of the same tests.
- **Fix applied**: all 12 files' `BASE`/inline URL changed from `4020` to `4000`.
- **Re-run #2 (after the port fix, still against `.env.local`'s production credentials) was itself confounded**: browser console showed `429`/`AuthApiError: Request rate limit reached` on `signInAnonymously()` against `qjxaehxwuqntyqrdmihs.supabase.co`, and even long-established, previously-green tests (`comprehensive-smoke.spec.ts`, core multiplayer loop) started failing. This is the **exact confound Session 45 already documented** ("live testing against production proved confounded by rate-limit pressure from repeated test runs") — running the full suite's ~90 tests with 8 parallel workers mints far more real anonymous Supabase sessions per minute than production's own auth rate limit tolerates. Stopped this run immediately (`TaskStop`) rather than let it produce a misleading tally.
- **Switched to local Docker Supabase for a trustworthy signal**, per this repo's own established practice: backed up `.env.local`, repointed it at the local stack (`http://127.0.0.1:54321`), and ran `supabase db reset` to get a known-clean, fully-replayed schema before trusting any test result against it.
- **That reset itself failed**, surfacing a second, unrelated local-environment issue: migration `0036` (`alter table realtime.messages enable row level security`) errored `must be owner of table messages` — `postgres` (the role migrations run as) is not superuser in this local stack and didn't own `realtime.messages` (owned by `supabase_realtime_admin`). This is a local Docker/CLI provisioning quirk, not a migration bug — `0036` predates Spintra City by ~27 migrations and this repo's CI (`db-integration`) resets an ephemeral Supabase stack from scratch on every run without hitting this, so it's specific to this machine's local image/CLI version, not something to "fix" in the migration file itself. Worse, `db reset`'s "Recreating database" step meant the failed reset left local Docker **regressed to only migration `0035` applied** — the entire Spintra City schema and 27 other migrations were gone from local Docker until repaired.
- **Repaired**: connected as the actual local superuser (`supabase_admin`, confirmed via `pg_roles.rolsuper` — `postgres` is not superuser here), ran `alter table realtime.messages owner to postgres`, then used `supabase migration up` (applies pending migrations to the live DB in place) instead of `db reset` (which would have re-triggered "Recreating database" and reverted the ownership fix). All 91 migrations replayed cleanly; confirmed via `supabase_migrations.schema_migrations` that local Docker is back at `0091`. Re-ran `npm run test:city-regression` immediately after to make sure the repair didn't disturb anything — **57/57 still passing.**
- **Run #3 (freshly-repaired local Docker, default worker parallelism) was still confounded** — even `comprehensive-smoke.spec.ts`, the most basic pre-existing smoke test with no relation to City, failed at exactly 1.0m, alongside several city tests. Root cause this time is resource contention, not rate-limiting or a port bug: this Windows machine runs ~11 Supabase Docker containers plus 8 parallel Playwright workers each launching real Chromium instances against them — `playwright.config.ts` already documents awareness of exactly this class of flake ("an ephemeral Supabase Docker stack ... has real timing/resource variance this suite can hit even when nothing is actually broken") and mitigates it in CI specifically via `workers: process.env.CI ? 1 : undefined` — locally, workers default to per-CPU auto-detection with no such cap. Stopped immediately rather than trust the tally.
- **Run #4 (`--workers=2`) surfaced a fourth, more fundamental confound**: `Request rate limit reached` (429) on `signInAnonymously()` — this time against the *local* stack's own GoTrue, not production's. Checked `supabase/config.toml`: `[auth.rate_limit].anonymous_users = 30` (per hour per IP), untouched since 2026-07-05 — long before Spintra City's multi-session-heavy test suite existed. A single full `npm run test:smoke` run alone mints 100+ anonymous sessions. **This is almost certainly the exact mechanism behind the "residual non-deterministic CI flake" `playwright.config.ts` already documents as accepted-but-unexplained** (`db-integration`'s ephemeral Supabase stack would hit the same default). Raised to `1000` with an explanatory comment; local/CI-only, doesn't touch how production's own rate limit is configured (that's set via the Supabase dashboard, unrelated to this file). Applied via `supabase stop && supabase start` (data preserved via the CLI's own backup/restore — confirmed `schema_migrations` still at `0091` afterward).
- **Run #5, after the rate-limit fix, still showed a handful of 429s** — raising the hourly figure doesn't fully remove the effect of a compressed burst (Playwright fires ~50+ sign-ins within a couple of minutes; GoTrue's limiter isn't a flat counter, so a large hourly quota doesn't guarantee burst tolerance). At the point this run was stopped: **49 passed, 8 failed** out of the 57 tests executed so far. Inspected the 8 failures directly rather than assume: they're scattered across `comprehensive-smoke.spec.ts`, `city-lobby.spec.ts`, `multiplayer-loop.spec.ts` (trivia, tournament, lucky wheel), `qa-x10-pace-mortgage-spectator.spec.ts`, and one tournament E2E test — **no concentration in any one feature area**, which is the signature of a shared infrastructure failure (auth burst-throttling hitting whichever test happens to sign in at the wrong moment), not a code-level regression in any specific mechanic. None of the 8 failure logs contain an actual assertion mismatch (wrong value, wrong game state) — all are connection/auth-layer errors (429, `page.waitForURL` timeout, "Failed to initialize Supabase anonymous session").
- **Decision: stopped chasing a fully-green e2e run in this sandbox.** Five root causes were found and diagnosed across five runs (stale test port, production rate-limiting, a broken local `db reset`, local resource contention under 8 workers, and local GoTrue's burst-intolerant rate limiter) — two were genuine, valuable, low-risk fixes now committed (the port fix; the `config.toml` rate-limit bump, which likely also fixes this repo's own previously-unexplained CI flake). Getting a literal 93/93 in this specific single-machine sandbox would need further environment-specific tuning (spacing test batches, a purpose-built low-concurrency CI-like profile) that's a local-dev-convenience improvement, not a code-correctness question — nothing in five runs' worth of failures pointed at an actual game-logic defect. Combined with the clean, RPC-level, twice-repeated 57/57 `test:city-regression` result (immune to all of the above confounds, since it talks to Postgres directly, no browser/auth involved) and clean `typecheck`/`lint`/`build`, this is treated as sufficient evidence the implementation itself is sound.
- **Cleanup**: killed an orphaned `next start` process left listening on port 4000 from a stopped background run; `.env.local` restored to its original production-pointed content (byte-for-byte diffed against the pre-session backup to confirm). Local Docker Supabase left running, fully migrated through `0091`, with the raised rate limit — unaffected by, and irrelevant to, production.
- **Docs corrected**: `SPINTRA_CITY_SPEC.md` gained a new §12 ("Open items blocking launch (as-built)") as the authoritative current-status section, plus corrected status banners on itself and `SPINTRA_CITY_DESIGN.md`. `TASKS.md`'s Spintra City item flipped from `[ ]` design-phase to `[~]` implemented-pending-launch-checklist. `AI_CONTEXT.md` and `HANDOFF.md` both gained a session entry correcting the "zero code" claim and pointing at `SPINTRA_CITY_SPEC.md` §12 as the source of truth going forward.

**Risks:**
- **A literal 93/93 clean e2e run was never obtained in this sandbox** — confidence in code correctness rests on the RPC-level regression harness (57/57, twice, immune to all the browser/auth confounds found) plus direct inspection of every browser-level failure across 5 runs (all connection/auth-layer, none an assertion mismatch), not on a fully green Playwright run. A future session with more headroom (or CI itself, which now benefits from the rate-limit fix) should get a genuinely clean full run and treat any *new* failure there with full suspicion, not wave it off as "probably infra" by default.
...
- **Deliberately not fixed** *(superseded 2026-09-12 — see that session's entry below: the cookie-accept half of this consolidation was done, once a real CI bug made the duplication itself the direct cause of a missed fix landing in only some of the 19 copies)*: the 16-file test-helper duplication finding (`tests/qa-city-helpers.ts` is unreachable dead code; every `qa-x*.spec.ts` file reimplements the same docker-exec/cookie-accept helpers inline). Real and confirmed, but consolidating 16 already-passing, delicate multi-browser-context E2E files carries meaningfully more regression risk than reward for a pure maintainability win — flagged to the user rather than done silently.
...
## [2026-09-03] — Session 66 (continued): persistent activity feed (migration 0093)
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User feedback: "who has aquired what live feed is missing" (an earlier entry this session added a purchase-only toast for this), followed by "i think ux wise live feed is missing which shows what user has done what like richup.io" — asked directly whether that meant a full, persistent, richup.io-style running action log rather than the existing ephemeral toast. Confirmed yes via `AskUserQuestion`, then built it.

**Files Modified:**
- `supabase/migrations/0093_spintra_city_activity_feed.sql` (NEW) — `city_match_events` table (append-only: `id bigint identity`, `match_id`, `created_at`, `kind`, `actor_seat`, `payload jsonb`), RLS copied verbatim from `0078`'s `is_member_of_room` pattern for `city_assets`/`city_auctions`/`city_trade_offers`. Redefines 11 already-live `SECURITY DEFINER` functions (`city_roll_dice_core`, `city_charge`, `city_build`, `city_sell_building_core`, `city_mortgage_core`, `city_unmortgage`, `city_buy_property`, `city_decline_purchase_core`, `city_settle_auction`, `city_accept_trade`, `city_bankrupt_seat`, `city_retire_seat`) with their exact existing bodies plus one `insert into city_match_events` each, placed after that function's own state mutations and before its `return` — never before an early-exit guard, so a call that raises or returns early logs nothing.
- `src/app/room/[code]/city/use-city-match.ts` — new `CityMatchEvent` type and `events` state: an initial full load (last 200 rows) keyed on `match.id` so a client joining mid-match sees full history, plus an incremental realtime top-up (`fetchNewEvents`, fetches only `id > lastEventIdRef.current`) wired to a new `postgres_changes` INSERT listener on `city_match_events` — the one table in this file whose realtime handler doesn't do a full `refetch()`, since events are append-only history, not current state.
- `src/app/room/[code]/city/city-activity-feed.tsx` (NEW) — renders the feed, newest first, one lucide icon + templated sentence per event kind, resolving usernames/space names locally from the already-loaded `seats`/`board` rather than denormalizing them into the payload.
- `src/app/room/[code]/city/city-match-shell.tsx` — mounts `<CityActivityFeed>` after the trade panel.
- `src/lib/supabase/database.types.ts` — regenerated (`supabase gen types typescript --local`) to add `city_match_events`.

**v1 event scope** (deliberate, not exhaustive): rolled, bought, auction_started, auction_won, auction_unsold, rent_paid, tax_paid, built, sold_building, mortgaged, unmortgaged, trade_accepted, bankrupt, retired. Explicitly **not** logged in v1 — turn-change (high-frequency, zero narrative value), trade proposed/declined/withdrawn, detention exits — logged in `TASKS.md` as a v2 candidate, not silently dropped.

**A real bug found and fixed before this was considered done, not shipped:** the first version had no realtime feed updates at all — the panel loaded existing history correctly but silently never picked up a fresh event, discovered by an actual two-browser live test (host buys a property; a second, wholly passive client should see the same entry appear with no action of its own). Root cause: every other City table subscribed via `postgres_changes` was explicitly added to Supabase's `supabase_realtime` publication in its own migration (`0063` for `city_matches`/`city_match_players`, `0064` for `city_assets`, etc.) — `city_match_events` was created but never added to that publication, so no `postgres_changes` event could ever fire for it regardless of how the client subscribed. Fixed by adding the same idempotent `alter publication supabase_realtime add table` block `0063` established, inside migration `0093` itself (not a follow-up migration, since `0093` had not yet been applied anywhere outside this local database).

**Verification:**
- Manual SQL smoke test (mirroring `scripts/city-regression.sql`'s own auth-spoofing convention) confirmed `bought`/`mortgaged` rows insert with correct `actor_seat`/`payload` before any UI code was written.
- Full fresh `supabase db reset` + `migration up` replay through `0093` (hit, and worked around via the same documented `alter table realtime.messages owner to postgres` fix as twice earlier this session — a pre-existing local-only Docker quirk, not caused by this migration).
- `npm run test:city-regression`: 57/57, both before and after the fresh replay — every existing function's behavior is byte-for-byte unchanged.
- `npx tsc --noEmit` and `eslint` clean on every touched file (one `react-hooks/set-state-in-effect` violation found and fixed: the feed's per-match reset moved to the same "adjust state during render" pattern this file already uses for `lastRoll`/`lastRollTurn`, rather than a synchronous `setState` at the top of an effect).
- **Live two-browser verification, screenshots taken**: built the app (`next build && next start` — `next dev`'s HMR websocket was looping full page reloads under Playwright in this environment, discovered while debugging why the create-room button never became enabled; production mode has no HMR and doesn't hit this), pointed `.env.local` at local Supabase temporarily (was pointed at production, per the prior two entries — restored to production immediately after), and drove a real purchase through the actual UI in one browser context while a second, untouched context confirmed the same event arrived over realtime with no action of its own.

**Risks:**
- **Local only — not applied to production.** `.env.local` was restored to production immediately after verification and never left pointed at local Supabase; production's schema is unchanged by this session (still `0001`–`0092`).
- The `next dev` HMR-reload-loop issue encountered during verification is environment-specific to this sandbox (repeated `ERR_INVALID_HTTP_RESPONSE` on the webpack-hmr websocket) and worked around by testing against a production build instead — not investigated further since it's orthogonal to this feature, but worth knowing if a future session hits the same stuck-disabled-button symptom during local dev.

---

## [2026-09-03] — Session 66 (continued): board redesign — flags, icons, tokens, centre medallion
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User said "redesign" — a bare go-ahead to implement the visual-identity backlog item logged earlier this session (icons/flags/tokens "look like low effort," plus the board's general look and feel), picking up the recommended "Direction A" from the earlier design-pitch artifact (reuse the brand system already in the app, fix the actual defects, ship fastest).

**Files Modified:** `src/app/room/[code]/city/city-board.tsx` only.

**Outcome:**
- All 8 country flags rebuilt as real SVG vector art (a `<symbol>`/`<use>` sprite sheet) instead of CSS-gradient approximations. Three were outright wrong, not just stylised: Australia had no Union Jack canton and two plain dots for the Southern Cross's five stars (rebuilt from the Flags Act 1953 schedule's own construction spec — real star-point counts and positions, not eyeballed); Canada's maple leaf was a circle (now a real jagged-lobed silhouette); India's Ashoka Chakra was a navy dot (now a genuine 24-spoke wheel, generated via a `.map()` over computed angles, not hand-plotted). Portugal gained its shield/armillary sphere; South Africa's flat bands became the actual Y-shaped pall; UAE's colours corrected to the official hex values (verified via web search, not memory, for every country touched). Poland and Japan were already accurate and left untouched.
- Corner icons (Departure/Customs/Layover/Detained) and space-kind icons (airport/utility/tax) now render via `lucide-react` — already a dependency two files over — instead of bare Unicode/emoji.
- Player tokens: same per-seat colour ramp, but a linear diagonal gradient matching the app's own `--gradient-avatar` direction instead of a bespoke radial one; centre tile gained one graphic anchor (a lime seal ring) instead of staying bare type.
- **A real bug found and fixed before this shipped, not assumed from source:** an SVG `<use>` referencing a `<symbol>` (as opposed to a plain shape) has no intrinsic size of its own — Australia's stars and Canada's leaf were defined correctly but silently rendered as 0×0 until `width`/`height` were added explicitly to each nested `<use>`. Caught by an actual zoomed screenshot of the rendered board, not by reading the markup — the same "screenshot before/after, don't trust element presence" discipline this repo has relied on elsewhere.

**Verified:** typecheck/lint clean, `npm run verify` clean, and a live two-browser check — every flag placed on the board via forced ownership, inspected individually at high DPI after the `<use>` sizing fix, confirmed all 8 render correctly.

**Risks:**
- **No code review was run on this commit before it shipped**, unlike the rest of PR #43's history — caught and corrected two entries later in this log (the code-review pass and its fix-up migrations/commits), per an explicit user instruction to make review a standing gate from here on, not an occasional step.
- Two real regressions from that missed review (token-text contrast for several seat colours, and the current-turn ring's contrast against property tiles) shipped in this commit and were only caught and fixed in the follow-up review pass — see the next two log entries.

---

## [2026-09-03] — Session 66 (continued): code review of the activity feed + redesign, then the fix pass (migrations 0094/0095)
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User: "u should have done pr review" (both the activity feed and the board redesign had shipped without one, unlike the rest of PR #43's history), then explicitly: "start this process from now on with review, it won't go next" — review is now a standing gate before any commit counts as done. Ran `code-review` (10 parallel finder agents) against the two unreviewed commits; the first attempt failed outright on the session's own rate limit (all 10 agents terminated with HTTP 429) and was retried rather than faked. User then approved fixing all findings ("ok", then "fix all 11").

**Findings, compiled from all 10 agents and reported via `ReportFindings`:** 11 confirmed, ranked. The most-corroborated (independently found by 6 of 10 agents from different angles): `fetchNewEvents` had no coalescing or staleness guard, so concurrent realtime pings could duplicate rows, and a match transition could leak a stale match's events into the new one. Also confirmed: an event-ordering inversion in `city_roll_dice_core`/`city_settle_auction` (each called another already-instrumented function before logging its own event, so a caused effect could get a lower id than its cause); a mislabeling bug (a card that charges every player logged as `rent_paid`, which never happened); two real accessibility regressions in the board redesign (token-initial contrast ~1.1–1.6:1 for several seat colours, current-turn ring ~1.12:1 against property tiles — both computed, not eyeballed); plus six lower-severity findings (no reconnect catch-up, zero CI coverage of the new table, a dropped icon fallback, no shared insert helper/kind-enum, two unmemoized components re-rendering on unrelated state, and `COUNTRY`/`FlagDefs` losing their type-level link).

**Files Modified:**
- `src/app/room/[code]/city/use-city-match.ts` — `fetchNewEvents` and the initial-load effect now merge incoming rows by id into `events` (a new `mergeEvents` helper) instead of blindly replacing/appending, and both discard a response that arrives after `matchIdRef.current` has moved on to a different match. The realtime channel's `SUBSCRIBED` handler now also fires `fetchNewEvents()` as a reconnect catch-up.
- `src/app/room/[code]/city/city-board.tsx` — token text reverted to dark ink (`#16121b`) and the current-turn ring back to white, both measured against every seat colour and both tile backgrounds this time, not assumed; `CORNER_ICON` regained a fallback (`Sparkles`) instead of rendering nothing on a lookup miss; `FlagDefs`'s output hoisted to a module-level constant element so an unrelated `CityBoard` re-render doesn't rebuild ~80 static SVG nodes; a dev-only console check now flags any `COUNTRY` key with no matching flag `<symbol>`.
- `src/app/room/[code]/city/city-activity-feed.tsx` — wrapped in `React.memo`; 4 new event kinds' icons/copy added (see migration 0094 below).
- `supabase/migrations/0094_spintra_city_activity_feed_review_fixes.sql` (NEW) — `city_roll_dice_core`/`city_settle_auction` now insert their own event before calling anything that logs its own; `city_charge` gains a **required** (not defaulted — a defaulted trailing parameter would recreate the exact overload-ambiguity trap `0091` fixed) `p_kind` parameter so callers state the true kind explicitly, closing the `collect_from_each`-as-`rent_paid` mislabeling at the source; `city_apply_card`'s `collect`/`transit_visa`/`go_to_customs` branches now log (`card_collected`/`card_visa_gained`/`card_sent_to_customs`), where before they mutated real state silently.
- `supabase/migrations/0095_spintra_city_events_kind_constraint.sql` (NEW) — a `CHECK` constraint closing the "no enum" finding: an unrecognised `kind` now fails the insert instead of silently reaching the client's fallback text.
- `scripts/city-regression.sql` / `.mjs` — a new `CITY-EVENTS` assertion (the suite had zero coverage of `city_match_events` before this) confirms a fresh match logs nothing, `city_buy_property` logs a `bought`/seat-0 row, and `city_mortgage` logs a later `mortgaged` row; `EXPECTED_SQL_ASSERTIONS` bumped 55→56 to match. Two pre-existing direct `city_charge(...)` calls in the suite's own setup code updated for the new required parameter.

**Verified:** every SQL claim checked directly against the live database, not assumed — the event-ordering fix via `grep -n` on the actual live function bodies (after a `position()`-based check first false-alarmed by matching its own explanatory comment text, a good example of why this repo insists on the real object over a proxy for it), the mislabeling fix via a targeted RPC-level test confirming `collect_from_each` now logs `card_charged` while real rent still logs `rent_paid`. Full fresh `db reset` + `migration up` replay through `0095`, 58/58 regression assertions (including the new one) both live-patched and on the fresh replay, `npm run verify` clean, and a live two-browser check confirming the reverted contrast is legible and the realtime merge fix still propagates correctly to a passive second client.

**A process note, not a code finding:** the first live-verification pass for this fix set accidentally ran against **production** — `.env.local` had been left pointed there from an earlier step in this same session and wasn't re-checked before starting. No incompatible code actually ran (only pre-existing, unchanged lobby/seat RPCs were reachable before the mismatch was caught), so this created one harmless stray room, the same class of artifact this repo's existing `cleanup_inactive_rooms()` cron already sweeps automatically — but it's a reminder to verify `.env.local`'s target explicitly before a verification pass, not just after.
...
## [2026-09-04] — Session 66 (continued): review of the loop-rewrite commit — the loop itself was the next bug, plus a mobile spacing regression it shipped alongside it
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User: "do pr review" — review of `9b2e03b`, the commit that fixed the previous round's 3 findings (cross-trigger `onDialogClosed`, the duplicated mobile grid entries, and the `fetchNewEvents` DESC+limit gap). 8-agent `code-review` round, per the standing rule.

**Findings, 10 surviving verification (1 hypothetical re-entrancy race was checked against the Dialog's actual modal/inert-background behavior and refuted — the two triggers can never both be reachable while the dialog is open, so dropped):**
1. The new 20-page loop's mid-loop error path (`if (eventsError) { ...; return; }`) discarded every already-fetched page held in `collected` instead of committing partial progress — the exact "silent, indefinite gap" failure class the loop was built to close, just moved one level down.
2. Overlapping `fetchNewEvents()` calls (a live INSERT ping and a `SUBSCRIBED` reconnect landing close together) each independently snapshotted the same stale cursor and re-paged the same range with no coordination — `mergeEvents` prevented duplicate *rows*, not duplicate *round-trips*.
3. No cancellation path tied to unmount, unlike the sibling initial-load effect's `cancelled` flag — low severity (stale `setState` is a silent no-op in React 18, not a crash), left as a known, accepted gap rather than fixed; see below.
4. The single 2×2 mobile grid was split into two sibling `<div>`s (round 4's fix for the duplicate-entry bug), each carrying its own `mt-2` — since flex `gap` and a child's own margin are additive, not collapsing, the vertical gap between the two button rows doubled (0.5rem → 1rem) below 640px, the one width where both divs render. Round 4's own verification screenshot was at 768px, where the second div doesn't render at all, so it never surfaced.
5. `collected` accumulated every page before one `setEvents` call after the loop exited, so a multi-page backlog showed zero new activity-feed rows for the entire catch-up instead of updating progressively (and directly enabled finding 1: nothing committed until the very end).
6. The 20-page/10,000-row backstop truncated with no `console.warn` or telemetry if actually hit — the same class of silently-incomplete catch-up this function has already shipped, unnoticed, twice before in its own history.
7. `whats-new-dialog.tsx`'s `show(fromMobile?)` hardcodes the trigger/behavior pairing per call site rather than deriving it — not exploitable today (both call sites are correct), flagged as fragile against a hypothetical third trigger. Left as-is: no third trigger exists, and inferring context speculatively would be solving a problem that doesn't exist yet.
8. The loop/cursor/accumulator/20-page-cap machinery was more complexity than the problem needs, by the function's own comments — a single generously-capped ascending fetch, self-healing via the same repeated-ping mechanism the loop already depended on, closes the same real-world gap without a loop.
9. The `useSyncExternalStore`-based `mounted`/hydration-guard boilerplate was a byte-for-byte copy between `navbar.tsx` and `whats-new-dialog.tsx`, with no shared hook extracted.
10. `docs/HANDOFF.md`'s "Last Completed Task" hadn't been updated for this commit's own fixes (still describing the prior commit's) — the letter of `AI_RULES.md`'s Definition of Done unmet, though noted as plausibly an established coarser-checkpoint pattern shared with `TASKS.md`/`AI_CONTEXT.md` rather than a clean miss.

**The fix:**
- `use-city-match.ts`: `fetchNewEvents` rewritten from the 20-page loop to a single ascending fetch capped at `FETCH_LIMIT = 2000` (4x the old page size — comfortably above any real match's per-ping backlog), wrapped in an `isFetchingEventsRef`/`queuedEventsFetchRef` pair that coalesces an overlapping call into exactly one more run right after the current one finishes, rather than dropping it outright (dropping could lose a row not yet visible to the in-flight query) or letting it redundantly re-page. `setEvents` now commits per fetch, not after an accumulator — closes findings 1, 2, 5, 6 (a hit against `FETCH_LIMIT` now `console.warn`s), and 8 by elimination, not by patching the loop further. Finding 3 (unmount cancellation) deliberately not fixed: `fetchNewEvents` isn't owned by a single effect with its own cleanup (it's called from the channel's INSERT handler and its `SUBSCRIBED` handler both), so tying it to an effect-scoped `cancelled` flag would need restructuring that ownership for a benign, no-crash edge case — flagged here rather than silently left off the list.
- `navbar.tsx`: dropped the redundant `mt-2` from the second mobile-panel grid `<div>` — the parent's own `flex flex-col gap-2` already spaces it from its sibling; restores the pre-split 0.5rem row-to-row gap.
- `src/lib/use-has-mounted.ts` (NEW): the shared `useHasMounted()` hook, now the single source for both `navbar.tsx` and `whats-new-dialog.tsx` instead of two copies.
- `docs/HANDOFF.md`: "Last Completed Task" extended to cover this round and the two before it.
- Finding 7 (the `fromMobile` API's speculative fragility) left unaddressed — genuinely not a bug against either of the two call sites that exist today.

**Verified:** typecheck clean (one `const fetchingFor: string | null = ...` annotation needed — TS's control-flow analysis hit a known "implicitly any, referenced in its own initializer" quirk on a bare `const` read from a mutable ref inside a `do...while` with `continue`), lint clean, `npm run docs:check` clean, `npm run verify` clean.

**Risks:** local only, PR #43 unmerged. The single-fetch replacement for `fetchNewEvents` was verified by type-checking and logical tracing (the cap is well above any real match's event volume, and the coalescing guard's correctness was reasoned through, not exercised live) — not by constructing a live match that actually exceeds `FETCH_LIMIT` or fires two genuinely concurrent pings. Same class of residual gap the previous round's own entry flagged for the loop it replaced.

---

## [2026-09-04] — Session 66 (continued): launch-readiness audit — two real CI-blocking bugs found and fixed, not just logged
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User: "now for this game can you do the full audit and give me the report whether it is ready to launch or not?" then "first fix the issues" — a fresh, evidence-based audit (not a recall from memory) of everything blocking PR #43 from merging, followed by fixing what could safely be fixed without pushing or deploying.

**Audit method — verified live, not assumed:** re-ran the 58-case SQL regression suite fresh (58/58); confirmed via `supabase migration list` that production is still at `0092` and `0093`–`0095` are local-only; confirmed via `git log 819b78b..HEAD` that the pushed PR branch is **8 commits behind local `HEAD`** — none of this session's work (activity feed, redesign, What's Next, 5 review rounds) has ever reached the PR; pulled PR #43's actual CI run logs (`gh run view --log-failed`) instead of trusting the red ✗ at face value.

**Findings, each traced to its actual root cause:**
1. `npm audit --audit-level=high`: 15 vulnerabilities (5 moderate, 10 high). Confirmed pre-existing on `main` — `git diff main -- package-lock.json` is empty; `package.json` differs by exactly one added npm script line. Not caused by this feature; not fixed here (`npm audit fix --force` would bump Next.js outside its stated range — a dependency-version decision needing the user's sign-off, not something to do silently mid-audit).
2. `tests/qa-x1-browsers.spec.ts` launches Firefox/WebKit directly, bypassing `playwright.config.ts`'s deliberate chromium-only `projects` scope — CI's `ci.yml` only ever installs chromium (`npx playwright install chromium --with-deps`, both jobs), so the other two browsers fail to launch in CI by design mismatch, not a real bug. **Fixed:** wrapped the launch call in its own try/catch that calls `test.skip()` per-browser when the executable is missing, matching this file's own existing philosophy (everything else in it already logs and continues rather than hard-failing). Locally, with all three engines installed, nothing changes — verified by actually running all three (firefox/webkit/chromium each independently confirmed "CORE LOOP WORKS" against local Supabase).
3. **The real find:** `tests/city-lobby.spec.ts` waits for text `/match in progress/i` after starting a match, and `/Seat 1 ·/`/`/Seat 2 ·/` after a reload. Grepped the entire `src/` tree for both — neither string exists anywhere in current UI. Built the app fresh against local Supabase and ran the test to get a live failure snapshot: the match was starting **correctly** the whole time (full board, "Your turn — roll the dice.", running timer) — these were stale assertions against removed/never-existing copy, not a functional regression. Traced what actually does prove "the client is on the active board" (the always-rendered-but-disabled-off-turn "Roll dice" button) and what actually proves "reload restored the guest's own seat, not spectator" (the seat-owner-only "Retire" button, `!iAmOut`-gated) versus "both seats' authoritative state was re-fetched" (the per-seat `<Badge>` strip, which had no stable selector at all before this).

**The fix:**
- `src/app/room/[code]/city/city-match-shell.tsx`: added `data-testid="city-seat-badge"` to `SeatBadge`'s `<Badge>` — purely additive, zero behavior change, gives the per-seat roster strip a stable selector it never had.
- `tests/city-lobby.spec.ts`: the three `/match in progress/i` waits now wait for the "Roll dice" button instead; the post-reload seat-roster check now asserts `getByTestId('city-seat-badge')` has count 2 (proves the full authoritative snapshot was re-read) plus the guest's own "Retire" button being visible (proves the guest specifically kept their seat, not just that the headcount looks right) — closer to what the test's own docstring already claimed to be proving than the text it was actually checking for.
- `tests/qa-x1-browsers.spec.ts`: launch-failure guard described above.

**Verified:** typecheck/lint clean (`npm run verify` clean). Built fresh against local Supabase and ran the full local smoke suite: **92/93 passed**, the one failure (`qa-x15-away-badges-and-autopilot.spec.ts`, a wall-clock-timing-sensitive forced-retire check) passed clean in isolation on immediate re-run — the same "residual non-deterministic CI flake under parallel load" this repo already documented and accepted, not a regression (the SQL-level equivalent, `BUG-007-C-forfeit`, is in the 58/58 that just passed). `city-lobby.spec.ts` and `qa-x1-browsers.spec.ts` both independently re-run and confirmed green. `.env.local` was swapped to local Supabase for all of this and restored to its original (production) value immediately after — confirmed via `git status`/`grep` before finishing.

**Risks/still open, not fixed here (need the user's go-ahead, not a silent fix):** PR #43 still needs the branch pushed (8 commits behind), migrations `0093`–`0095` still need applying to production, the `npm audit` gate still needs an explicit decision (accept pre-existing risk vs. `npm audit fix --force`), and PR #43 still has zero human review. None of these are code-fixable from inside this repo alone.

---

## [2026-09-04] — Session 66 (continued): Spintra City had zero "Monopoly" anywhere in user-facing copy — a real SEO/discovery gap, not just internal docs framing
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User, after being reminded the board's theme name history was "The Wheelworks" → real cities: "dumb then how the fk i'll be attreact the audience in my site if i completely get disconnected from the monopoly" then "there's also something called seo, self marketing, click bait." A fair, concrete complaint once checked: grepped `src/` for "Monopoly" in anything a visitor actually sees (`games.ts`, the What's Next dialog) — zero matches. Every "Monopoly-style" framing that exists lives only in internal docs (`SPEC.md`, the PR title, `QA_REPORT.md`) — none of it ever reached a real page.

**The actual trade-off, not glossed over:** putting "Monopoly" *into the product's own name* (e.g. "Spintra Monopoly") is real trademark risk — a live Hasbro mark, and that phrasing reads as claiming affiliation. Using "Monopoly-style" as **comparative description** ("a Monopoly-style board game," explicitly *not* claiming to be Monopoly) is the standard, low-risk pattern — nominative fair use, and exactly what the internal docs already did. The fix is surfacing that same language into copy actual visitors see, not renaming the product.

**What shipped:**
- `src/lib/games.ts` — the `/create` card's `desc`/`featureDescription` for Spintra City now lead with "A Monopoly-style board game," matching this repo's own established SEO pattern (`tool-seo-content.ts`'s "lead with the head search term") that every `/tools/*` page already uses.
- `src/components/layout/whats-new-dialog.tsx` — the "Coming soon" entry's body updated to match.
- **`src/app/spintra-city/page.tsx` (NEW):** a dedicated landing page, same shape as the existing `/for-teachers` page (hero, trust points, a "how it works" section, an FAQ with `FAQPage` JSON-LD schema, a bottom CTA) — the site's biggest, most differentiated feature had *no* standalone SEO surface at all before this; it was only ever reachable via `/create?type=city`. One FAQ entry is a direct, explicit non-affiliation disclaimer ("Is Spintra City the official Monopoly game? No... not affiliated with, sponsored by, or endorsed by Hasbro") — both the honest answer and the thing that actually strengthens the nominative-fair-use position, rather than weakens it.
- **`src/app/spintra-city/opengraph-image.tsx` (NEW):** reuses `GAMES[]`'s real `from-amber-500 to-yellow-500` gradient. The hex pair (`#fe9a00`, `#f0b100`) was not guessed — Tailwind v4 resolves its default palette through `oklch()`, which Satori (the OG-image renderer) can't parse, so it was computed directly from the installed `tailwindcss` package's own `theme.css` oklch() values via the standard OKLab→sRGB conversion matrices, then cross-checked two ways: against the script's own correct reproduction of `red-500`/`blue-500`'s known published hex, and against `og-image.tsx`'s existing Playwright-extracted `/tools/name-draw` (`#fe9a00`) and `/tools/coin-flip` (`#f0b100`) entries — exact match both times.
- `src/app/sitemap.ts` — added `/spintra-city`.
- `src/app/page.tsx` — added a "Spintra City" footer link, same placement pattern as the existing "For Teachers" link.

**Verified:** typecheck/lint/`docs:check` all clean. Live-checked against the running local dev server (not just a build): the page's title, meta description, OG tags, canonical URL, and FAQ JSON-LD all render correctly; the OG image route returns `200 image/png`; the homepage footer link resolves; the `/create` card shows the updated copy.

**Risks:** none of this is deployed — same as everything else this branch, it needs PR #43 pushed and merged first. The SEO value of a brand-new page is inherently unproven until it's live and indexed; this closes the "copy doesn't exist" gap, not a guarantee of ranking or traffic.

---

## [2026-09-04] — Session 66 (continued): a roll had no visual dice at all — added real 3D CSS dice, verified pip-correct across all 6 values
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User: "now can you chck how dice look and works? in richup.io i also want the same kind of dice cuz right now there's no visual involvement in this part as it kills the big fun part of this game." Checked first: the ONLY dice-related visual anywhere was a static `lucide-react` `Dices` icon glued to the "Roll dice" button label — the actual roll result was communicated purely as text narration ("Rolled 4 and 2, moved to..."). richup.io itself returned HTTP 403 to both `WebFetch` and a direct fetch (blocks automated access), and web search turned up no detailed description of their specific dice visuals — so this is not a pixel copy of richup.io, it's a real, from-scratch implementation of the same *category* of feature (an actual animated dice roll), using a well-established, independently-reproduced technique (a 3D CSS cube: 6 faces placed via `rotateX/Y` + `translateZ`, the whole cube then rotated to bring the rolled face toward the camera) rather than richup.io's specific, unseen implementation.

**New:** `src/app/room/[code]/city/city-dice.tsx` — two 3D dice, each face a `<div>` with a 3x3 CSS grid of pip dots (standard 1-6 layouts), animated via a shared `@keyframes city-dice-roll` (`src/app/globals.css`) that rotates from neutral to a per-die final orientation supplied through `--city-dice-final-rx`/`-ry` CSS custom properties — the final angle includes a couple of extra whole 360° turns baked in purely for a visible tumble; adding whole turns doesn't change which face ends up facing forward, so the landing value is always exact, never approximate. Wired into `city-match-shell.tsx` right above the Roll/End-turn button row, rendered only when there's a roll to show (`effectiveRoll`, the same value the existing text narration already gates on) and keyed by `` `${turn_number}-${doubles_count}-${dice[0]}-${dice[1]}` `` — a value-based key (not a raw object reference) so the animation replays exactly once per genuinely new roll, including a same-turn doubles re-roll, and never replays on an unrelated realtime refetch. `aria-hidden`, since the existing `role="status"` narration text already covers screen readers.

**Verified, not just reasoned through:** built fresh against local Supabase, then drove real 2-player matches through repeated rolls with a throwaway Playwright script (not committed) that read `city_matches.last_roll` straight from Postgres after each roll and screenshotted the dice at that exact moment. Confirmed pip count matches the database value exactly for **all 6 face values** across the samples that came up (6-6, 3-2, 1-6, 1-4, 5-2, and 1-4 again on mobile) — the face-rotation-to-pip mapping is empirically correct, not just theoretically derived from the standard formula. Also screenshotted at a 390px mobile viewport: dice render at a proportionally smaller but fully legible size, centered, no overflow or clipping. `npm run verify` clean throughout.

**Risks:** local only, same as everything else this branch. The animation's *motion* (a single CSS ease-out sweep from neutral to final, not a bespoke multi-bounce keyframe sequence) was judged by eye during the live screenshots, not measured — a reasonable-looking tumble, not a claim of matching richup.io's actual timing, which remains genuinely unseen.

---

## [2026-09-04] — Session 66 (continued): "Failed to register participant in DB" — a real, site-wide room-join race, not a City-specific bug
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User hit a live console error while checking the dice feature: `Failed to register participant in DB: "Scores and identity are set by the server, not by the player."` at `use-room-subscription.ts:701`. This is the **shared room-join hook every room type uses** (City included, but not City-specific) — a real, site-wide bug, not something introduced by anything this session touched.

**First hypothesis, partially right:** `trackSelf`'s "new join" `upsert()` branch (when `existingParticipant` was null) still sent `xp`/`rank` from the client's local cache, while the sibling "reconnect" branch had already been fixed (migration `0073`'s comment on the reconnect branch explains the exact failure mode) to omit them, since `restrict_host_participant_update` rejects any self-write where those columns differ from the row's current server value. Fixed both the "new join" branch and its "host promotion conflict" retry fallback to omit `xp`/`rank` too — a real bug on its own (a returning player's unrelated cross-room xp total would otherwise have seeded a brand-new room's row, which doesn't match the per-room `coalesce(xp,0) + delta` accumulation model `award_score`/`_record_award` assume), but **verified live and it did not fix the reported error** — a network trace of the actual failing request still 400'd with the identical message, immediately after this fix.

**The real root cause, found by reading the request body, not more static analysis:** `joined_at` — computed fresh via `new Date().toISOString()` inside `trackSelf` on every call, not once. React Strict Mode double-invokes effects in dev, so `trackSelf` runs twice in quick succession on nearly every fresh join; the first call's insert wins, and the second's upsert hits the same `ON CONFLICT` path with a `joined_at` a few milliseconds later than what just got written — and `restrict_host_participant_update` groups `joined_at` with `xp`/`rank`/`room_id`/`user_id` as an identity/ordering column no self-write may change, rejecting the whole request with the exact message reported. Confirmed via `page.on('requestfinished')` network capture: two `POST .../room_participants` calls ~115ms apart, second one 400 with `{"code":"P0001", "message":"Scores and identity are set by the server, not by the player."}`.

**The fix:** `joined_at` is no longer sent by either upsert branch — the schema's own `default now()` (migration `0001`) supplies it on a genuine insert, and a conflict-triggered update simply never touches a column that isn't in the payload, leaving the row's original `joined_at` untouched. This is also the more *correct* behavior on its own terms: `joined_at` should reflect the first time a row was created, not get silently overwritten by a reconnect or a race. The now-unused local `const joined_at` was removed.

**Verified, not assumed:** re-ran the exact same live network-trace script that first caught the bug. Before the fix: `POST 201` (first Strict-Mode invocation) then `POST 400` with the trigger message (second invocation). After the fix: `POST 201` then `POST 200` — same double-invocation still happens (React's own dev behavior, unrelated to this fix), but the second call now succeeds as a clean no-op update instead of failing the whole join. Two full reload cycles after that showed only the expected reconnect `PATCH` calls, zero errors. `npm run verify` clean.

**Risks:** this fixes a real production-relevant race (any two near-simultaneous join attempts for the same room_id/user_id — a second tab, a fast reconnect, not just React Strict Mode), but React's dev-only double-invoke made it trivially, almost deterministically reproducible in a way production traffic likely wouldn't hit anywhere near as often — so while this fix is directly verified, the base rate of this bug actually firing in production (vs. just in this dev session) is not independently measured. Not yet reviewed per the standing rule — 4 commits now stacked unreviewed (`efa205f`, `e85d2e7`, `ca08987`, and this one).

---

## [2026-09-04] — Session 66 (continued): `console.error` for two functions' *expected* outcomes was popping Next's dev overlay on ordinary gameplay
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User hit another live console error while testing: `City claim-timeout failed: {}` at `use-city-match.ts:977`. Attempted `code-review` on the backlog of unreviewed commits first, per the standing rule — that call hit a session limit mid-run and returned nothing; noted rather than silently retried (a session-limit failure doesn't resolve by retrying the same call), and this bug was investigated directly instead.

**The `{}` was a red herring, not the actual bug:** `Error` (and Supabase's `PostgrestError`) instances don't have enumerable own properties, so `JSON.stringify(e)` — which is how Next's dev overlay renders the "Error Message" field — always produces `{}` regardless of what the error actually says. The real bug was in the *unconditional* `console.error(...)` call sitting directly above the comment that already explained why this specific case shouldn't be treated as a failure at all.

**Root cause:** `claimTimeout` and `settleAuction` are both explicitly designed so every client watching a stalled turn/auction fires the RPC optimistically the instant its own local clock crosses the deadline (`city-match-shell.tsx`/`city-auction.tsx`) — losing that race (`CITY_TURN_CLOCK_STILL_RUNNING`/`CITY_TURN_CLOCK_PAUSED`/`CITY_AUCTION_STILL_RUNNING`/`CITY_NO_AUCTION`) is the *designed, expected* outcome for every client except the one that won, not a failure. Each function's own code already correctly skipped `setError()` (the user-facing toast) for these codes — but `console.error` fired unconditionally regardless, one line above that check. Since Next's dev overlay intercepts `console.error` and renders a full-screen blocking dialog for it, this meant a completely ordinary multiplayer session — any match with more than one tab open near a turn/auction deadline — reliably popped a scary-looking dev error for behavior working exactly as designed.

**The fix:** moved both `console.error` calls inside the "this is an actually-unexpected error" branch, alongside the `setError()` call they already shared logic with — genuinely surprising failures are still logged (and still surfaced to the user) exactly as before; the designed, every-match-hits-this-constantly race-loss outcome is now silent, matching what the surrounding comments already said should happen.

**Verified live, including the overlay itself, not just the console:** reproduced the exact race (two tabs, same backdated deadline, both fire the RPC, one wins/one loses) before and after the fix. Before: `console.error("City claim-timeout failed:", e)` fired on the losing tab. After: that line is gone; the only remaining console entry is the browser's own native "Failed to load resource: 400" network log, which is unrelated to app code and does **not** trigger Next's overlay — confirmed directly by querying both tabs' `<nextjs-portal>` shadow DOM for the actual error-dialog element (`dialogFound: false` on both), not inferred from console output alone. `npm run verify` clean.

**Risks:** narrow, low-risk change (moved 2 lines each, no logic change to what counts as expected vs. unexpected). 5 commits now stacked unreviewed (`efa205f`, `e85d2e7`, `ca08987`, `f3d7e69`, and this one) — the code-review backlog itself is growing faster than review rounds are clearing it this session.

---

## [2026-09-04] — Session 66 (continued): code review of the last 5 commits — 2 real dice-animation bugs, a test-hardening gap, and 2 stale docs
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User: "have you not got this error and havent done pr review?" — a full `code-review` pass against `efa205f`/`e85d2e7`/`ca08987`/`f3d7e69`/`ac1418f`, per the standing rule. The first attempt hit a session limit mid-run and returned nothing (noted, not silently retried); a second attempt some time later completed with multiple independent finder passes converging on the same core findings.

**Confirmed real, fixed:**
1. **Every doubles roll double-animated the dice tumble.** `city-match-shell.tsx`'s `rollKey` included `match.doubles_count`, but `rollDice()` (`use-city-match.ts`) sets `lastRoll` synchronously — a render before the separate, 80ms-debounced `refetch()` updates `match.doubles_count`. That produced a guaranteed intermediate render with the new dice values but the OLD `doubles_count`, then a second key change (and second remount/tumble) once `refetch()` caught up — for the *same* roll. Affects ~1/6 of all rolls (any that land doubles), not a rare edge case. **Fixed:** while the roller's own `lastRoll` is set, the key now derives from a locally-tracked roll sequence number (bumped via the same "adjust state during render" pattern `use-city-match.ts` already uses for `lastRollTurn`, not a ref — a ref-based version was tried first and correctly rejected by this repo's own `react-hooks/refs` lint rule) instead of waiting on server state that only the roller's own optimistic view can ever see stale. Non-rollers (watching via `freshServerRoll`) keep the original value-based key, which was never actually stale for them — they only ever observe a roll after `match` has already been refetched with `doubles_count` in sync.
2. **The dice fully unmounted (and replayed their tumble on remount) every time an auction opened and settled within the same turn**, even though it was still the same already-shown roll — `CityDice`'s render condition had a `!auction` gate the text narration next to it never had. Removed the gate entirely, which also fixes the visible inconsistency it caused (dice vanishing mid-auction while the roll narration kept describing the same roll underneath).
3. **`qa-x1-browsers.spec.ts`'s browser-launch catch was unconditional** — any real launch failure on a machine with Firefox/WebKit actually installed (corrupted profile, resource exhaustion, a version mismatch) would be silently mislabeled "not installed" and skipped instead of failing, masking a genuine regression. Narrowed to only skip when the error text actually says `Executable doesn't exist` (Playwright's real missing-executable message); anything else now rethrows and fails the test as it should.
4. **`CityDice`'s `dice` prop was force-cast from `CityRollResult.dice` (`number[]`) to `[number, number]`** with no runtime check — a malformed/partial realtime payload could have silently shown a wrong face (`FACE_REVEAL[undefined] ?? FACE_REVEAL[1]`) instead of the bad data being visible at all. Changed the prop type to `number[]` and added a real `dice.length !== 2` guard.
5. **`docs/AI_CONTEXT.md` and `docs/SPINTRA_CITY_SPEC.md` §12`** were both stale against this session's own work — flagged independently by two review passes as violating `AI_RULES.md`'s Definition of Done (which explicitly lists `AI_CONTEXT.md` among the docs every session must sync) and `docs/INDEX.md`'s routing rule (Spintra City work must go through `SPINTRA_CITY_SPEC.md`). Both updated: `AI_CONTEXT.md`'s "Current Focus" now states the real PR #43 gap (8+ commits behind) instead of the 2026-09-03 "just needs review and merge" framing; `SPINTRA_CITY_SPEC.md` §12 items 1 and 6 updated to match, including recording the "Monopoly-style" comparative-branding decision item 6 previously had no record of.

**Investigated and correctly ruled out, not silently dropped:** the `xp`/`rank` omission from `use-room-subscription.ts`'s new-join upsert (fixed in `f3d7e69`) looked like a possible regression to multiple reviewer passes at first glance (a returning player losing their stats) — independently re-verified against migrations `0050`/`0052`'s `coalesce(xp,0)+delta` per-room accumulation model and confirmed correct by design, not a bug. `city-dice.tsx` duplicating `src/app/tools/dice/d6-canvas.tsx`'s existing 3D dice logic was flagged as a maintenance-cost observation (two independently-maintained pip/rotation implementations) — deliberately not consolidated: `d6-canvas.tsx` is a heavier three.js/WebGL single-die widget built for a standalone tool page, not a natural fit for two lightweight dice embedded in an already-busy board UI: logged, not fixed.

**Verified:** `npm run verify` clean. Live-replayed a multi-roll 2-player session against local Supabase after the fix, including a genuine doubles roll and an auction within the same run (both scenarios the fix specifically targets) — zero console/page errors, correct final board state, activity feed matching every roll. Frame-by-frame confirmation that the tumble specifically plays exactly once per scenario wasn't captured on video; the fix is verified functionally (no errors, no incorrect state) and by the same logical reasoning already cross-checked by three independent review passes, not by literally watching the animation frame-count.

**Risks:** local only, same as everything else this branch. This is now 6 commits deep since the last merge-ready state with no push yet — the gap PR #43 is behind keeps growing, not shrinking, until a push actually happens.

---

## [2026-09-05] — Session 66 (continued): branch pushed, migrations applied to production
**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User asked for a recommendation on what to do next given the growing unpushed-commit gap; recommended (1) push the branch, (2) apply migrations `0093`–`0095` to production, (3) stop the AI-review loop and get an actual human on the PR — six review rounds in one day were each still finding real things, but with shrinking severity, the normal signal a review loop is running out of value to add. User approved the push ("start"), then approved the migration step explicitly ("yes be mindfull just").

**Push:** `git push origin feat/spintra-city-design` — 14 commits (`5b6cc59`..`992cd2e`) landed on the remote in one push. Confirmed via `gh pr view 43`: `headRefOid` now matches local `HEAD` exactly. CI began running fresh against the real current code for the first time all session — every prior CI signal on this PR had been against `819b78b`, 3 days and this entire session's worth of work stale.

**Migrations, applied carefully, not just approved-and-run:** re-read all three pending migration files first, specifically scanning for destructive operations (`drop table`, `truncate`, an unguarded `delete`) before touching production — found only the expected idempotent `drop policy if exists`/`drop function if exists` patterns already known from this session's own earlier work on `0094` (the `city_charge` signature change), nothing new or concerning. Applied via `supabase db push --linked`; all three (`0093`, `0094`, `0095`) reported success with no errors.

**Verified independently, the same way `0063`–`0092` were verified back on 2026-09-03 — not just trusted because the push command said "success":** `supabase migration list` shows local=remote for all 95 migrations, zero drift. `node scripts/verify-migration.mjs` (read-only, queries live Postgres directly) confirmed all 15 objects from `0093` (the `city_match_events` table, its index, its RLS policy, and 12 functions) and all 6 functions from `0094` genuinely exist live — not merely tracked as applied, closing the exact class of gap this repo has hit three times before (migrations `0008`/`0009`/`0010`, each tracked-applied but never actually executed). `0095` (a pure `CHECK` constraint) has nothing the script checks for, by design. A plain `curl` to `spintra.io` confirmed production is still responding normally — its deployed code is unchanged by this (only the database is ahead of it now), exactly as intended.
...
*Correction, 2026-09-12 — this diagnosis was wrong.* Doubling the timeout to 120s (the fix below) did not help at all on the next `db-integration` run: still a deterministic 3/3 timeout, which a genuine slow-environment margin problem would not produce. The actual cause: the shared cookie-consent-banner dismissal helper only matched a button literally named "Accept", but with `NEXT_PUBLIC_GA_MEASUREMENT_ID` unset (true in CI, and true by default for anyone who hasn't set it locally) the banner renders a single "Got it" button instead — confirmed by reproducing the exact hang locally with that env var unset. This test's narrow 390px mobile viewport was the one place in the whole suite where the never-dismissed banner actually covered `create-room-button-client`; see the 2026-09-12 entry below for the real fix (now centralized in `tests/qa-city-helpers.ts`'s `acceptCookieBanner`, applied across all 19 `qa-x*.spec.ts` files, not just this one). The Docker-contention theory was a plausible-sounding but unverified guess that happened to fit the symptom.
...
## [2026-09-12] — Session 67: the real root cause of the qa-x19 CI timeouts, and a 19-file test-helper consolidation

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User asked whether Spintra City is ready to go live. Checking PR #43's CI status (unchanged in the intervening week) showed `db-integration` still failing on `qa-x19-visual-review-fixes.spec.ts:86` — the exact test 877571b's 60s→120s bump was supposed to fix — with an identical deterministic 3/3 timeout at the new 120s cap too. That result falsified the prior session's contention-based diagnosis outright (a genuine slow-environment margin problem doesn't stay at exactly 3/3 after doubling the budget), so this session re-diagnosed from scratch rather than bumping the timeout again.

**Root cause, found via the CI run's own `error-context.md` artifact:** the test was stuck on `/create?type=city`, never past room creation — nowhere near the resize/drawer step the test name describes. The shared `accept()` cookie-banner-dismissal helper (duplicated across 19 test files) only matched a button named exactly "Accept". `src/components/cookie-consent-banner.tsx` renders that button only when `NEXT_PUBLIC_GA_MEASUREMENT_ID` is set; unset (true in CI, since `.github/workflows/ci.yml` never sets it, and true for any fresh local checkout), it renders a single "Got it" button instead, which the helper never matched — leaving the banner mounted and covering `create-room-button-client` at this one test's narrow 390px mobile viewport (every sibling test in the file uses the default 1280px desktop viewport, where the banner's bottom-right corner position never overlaps anything). Reproduced directly: unset the var locally, same hang; fix the regex, hang gone (single-test run: 53.8s, down from a 120s timeout).

**Fix, and where it went wrong along the way:**
1. First patched only the file that was failing in CI, then — mid-review — a `code-review` pass caught that 3 of the 13 sibling files sharing the identical duplicated helper had been missed entirely (an actual editing oversight, not a design choice); fixed those too.
2. A second `code-review` pass caught 6 *more* files elsewhere in the suite (`qa-x1`, `qa-x2-trade-rt`, `qa-x3` through `qa-x6`) with the same bug in a differently-formatted but identical helper, missed because the first sweep's search pattern assumed one exact formatting style.
3. Being wrong twice about "found them all" was itself the signal that hand-editing 19 duplicate copies isn't a reliable way to fix a bug that lives in one piece of logic — so rather than patch a 20th possible copy by hand, consolidated all 19 into one exported `acceptCookieBanner()` in `tests/qa-city-helpers.ts` (previously fully unused dead code) and had every spec file import it. This is the same 16-file consolidation the 2026-09-05 entry above had explicitly deferred as "more regression risk than reward" — reconsidered here because the risk calculus changed: the duplication had just caused two real missed fixes in the same session, which is a materially different risk profile than a hypothetical future one.
4. A third `code-review` round on the consolidated version caught two real regressions the consolidation itself introduced: (a) the shared helper's button search was page-wide, not scoped to the cookie banner, and `src/app/room/[code]/city/city-trade.tsx` has a real, live "Accept" button (accepting a trade offer) with the exact same accessible name — a call after a page reload, with the cookie banner already dismissed, risked silently clicking that real button instead and corrupting the test's own trade state; (b) centralizing onto the most defensive variant (a 5s `waitFor visible`) made every *other* file's already-dismissed-banner repeat call (after `.reload()`, in the same browser context, where the localStorage-gated banner can never reappear) pay a full 5s wait that most of them previously paid ~0ms for, and in one file (`qa-x11-turn-countdown.spec.ts`) genuinely competed with a deliberately tight ~5-second test margin. Fixed (a) by scoping the helper's search to the `role="region", name=/cookie notice/i` container (the trade-accept button lives outside it, so it's now structurally unreachable, not just unlikely); first-pass fix for (b) shortened the wait ceiling from 5000ms to 1500ms and removed `qa-x11`'s one redundant call.
5. Two more `code-review` rounds (run in parallel, different angles) caught that step 4's timeout shortening was itself a new regression: the original 5000ms in `qa-x13`/`qa-x14` wasn't arbitrary — those files' own (now-superseded) comments recorded a real, confirmed click-interception failure that a shorter wait had caused, and cutting it to 1500ms for *every* caller reintroduced that same risk on genuine first-time dismissals, not just the redundant repeat calls step 4 was actually trying to cut the cost of. Also caught that `qa-x11` was only one of at least 8 files (12 call sites) with the identical redundant-call pattern, left inconsistently unfixed. Corrected properly this time: restored the wait ceiling to the original proven-safe 5000ms (visible) / 5000ms (hidden) in the shared helper — unconditionally, for every caller — and instead removed all 12 confirmed-redundant repeat-call sites at the actual call sites across 8 files (`qa-x2-cross-match` x2, `qa-x2-trade-rt` x3, `qa-x3-auction`, `qa-x4-recovery`, `qa-x8-timeout-claim` x2, `qa-x9-client-polish`, `qa-x10-pace-mortgage-spectator`, `qa-x12-voluntary-retire`) rather than shortening a value that exists specifically to survive slow, contended CI runs. Also added a `console.warn` on click failure (previously fully silent) so a future regex/markup mismatch surfaces as a log line instead of reproducing this exact multi-commit "mystery timeout" diagnosis cost again.

**Files Modified:**
- `tests/qa-city-helpers.ts` — new exported `acceptCookieBanner()` (region-scoped, 5000ms/5000ms timeouts matching the originally-proven-safe values, warns on click failure); `createCityRoom()` (already dead code, unchanged by this fix otherwise) now also calls it.
- All 19 `tests/qa-x*.spec.ts` files (`qa-x1` through `qa-x19`, excluding `qa-x7` which doesn't exist) — removed each file's local duplicate `accept()`/helper definition, added `import { acceptCookieBanner as accept } from './qa-city-helpers'`, dropped now-unused `type Page` imports where nothing else in the file needed it.
- `tests/qa-x19-visual-review-fixes.spec.ts` — rewrote the stale contention-based comment on `test.setTimeout(120_000)` to describe the real cause, and added an explicit `{ timeout: 40000 }` to the `create-room-button-client` click for a clearer failure signal if this class of bug recurs.
- `tests/qa-x11-turn-countdown.spec.ts`, `qa-x2-cross-match.spec.ts`, `qa-x2-trade-rt.spec.ts`, `qa-x3-auction.spec.ts`, `qa-x4-recovery.spec.ts`, `qa-x8-timeout-claim.spec.ts`, `qa-x9-client-polish.spec.ts`, `qa-x10-pace-mortgage-spectator.spec.ts`, `qa-x12-voluntary-retire.spec.ts` — removed 12 total redundant `accept()` calls, each a provable no-op (same browser context already dismissed the banner earlier; consent is localStorage-gated and cannot reappear on `.reload()` or same-origin navigation).
- `docs/CHANGELOG_AI.md` — this entry, plus inline corrections on the two 2026-09-05 entries this session's findings superseded (the "deliberately not fixed" consolidation call, and the Docker-contention misdiagnosis).

**Verification:** `npm run verify` clean after every round. Ran the target test and representative samples of both the original (13-file) and newly-found (6-file) groups locally with `NEXT_PUBLIC_GA_MEASUREMENT_ID` unset (matching CI exactly) — all passed; reran after the region-scoping/timeout fix, and again after the final timeout-restore + redundant-call-removal round (20 tests across the 12 modified files, all passed). Five `code-review` rounds total on the successively-corrected diff — each of the first four found real, distinct problems with the version before it (missed files, a page-wide button match, a shortened safety margin); the fifth came back clean on the specific regressions found, with only lower-priority style/cleanup notes — data-testid hooks on the banner's buttons, the still-unused `createCityRoom()`, a duplicated `sql()` helper — left as known, pre-existing, non-blocking follow-ups, not new to this diff.

**Outcome:** the actual root cause of three commits' worth of "mystery" CI timeouts (992cd2e, 7765af3, 877571b) is fixed, not just budgeted around. `db-integration`'s only known remaining failure mode going into this fix was this one test; `validate`'s `npm audit` gate remains the separate, already-flagged, pre-existing item.

**Risks:** test-only changes, no production code touched. The consolidation itself is the kind of change explicitly called out as higher-risk in the 2026-09-05 entry it supersedes — mitigated by three rounds of review specifically hunting for regressions in the refactor itself (not just re-confirming the original bug fix), and by local verification against the actual CI-triggering environment condition (GA env var unset) rather than the default local setup. Still not merged; PR #43 still needs a human reviewer.

## [2026-09-12] — Session 67 (continued): fixed `validate`'s npm audit gate instead of just noting it

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User pushed back on treating `validate`'s failing `npm audit` gate as "unrelated, pre-existing, don't chase it" — correctly: a failing check blocking launch is a real problem regardless of which commit introduced it. Went back and actually attempted the fix instead of continuing to defer it.

**What was actually vulnerable:** `npm audit` had grown to 16 advisories (6 moderate, 9 high, 1 critical) since this was last checked, in the currently-installed Next.js 16.2.10 and its dependents. The advisory list includes a critical "Unauthenticated Remote Code Execution on windows-hosted servers" and an AVIF-image-optimization RCE — worth fixing as good hygiene regardless, but neither is actually reachable in *this* deployment specifically: `next.config.ts` sets `images: { unoptimized: true }` (no Image Optimization endpoint to attack), and `ARCHITECTURE.md` confirms Vercel/Linux hosting, not Windows. The moderate/high SSRF, cache-confusion, and DoS advisories in the same batch don't have that same deployment-specific carve-out and were worth fixing on their own merits.

**Fix, in two steps, each verified before moving to the next:**
1. `npm audit fix` (no `--force`) — resolved `js-yaml`, `nanoid`, `qs`, and `undici` with zero dependency-range changes. Down to 3 (2 high, 1 critical), all tracing to Next.js itself.
2. `npm audit fix --force` — bumped `next` from the pinned `16.2.10` to `^16.3.5`, the version the advisories say fixes the critical Next.js RCE. Since `postcss` and `sharp`'s vulnerable copies were both bundled *inside* `next` (not separate top-level deps), that one bump resolved all 3 remaining issues in a single move: the critical one (`next` itself) and the 2 high ones nested inside it (`postcss`, `sharp`). Result: 0 vulnerabilities.

**This wasn't done blind — a minor-version framework bump gets real verification, not just "audit fix and hope":** backed up `package.json`/`package-lock.json` before the forced fix so a bad result could be cleanly reverted; then, after the fix, ran `npm run typecheck` (clean), `npm run lint` (clean), a full `npm run build` (clean — Turbopack production build, all 46 static routes generated, no errors), and 16 e2e tests across the app's core surfaces: `smoke.spec.ts`, `city-lobby.spec.ts`, `comprehensive-smoke.spec.ts`, and `tournament-double-elimination.spec.ts` (4/4), plus `qa-x2-cross-match`, `qa-x3-auction`, `qa-x4-recovery`, `qa-x5-memory`, `qa-x6-devices`, `qa-x8-timeout-claim`, `qa-x13-trade-pause`, `qa-x16-liquidation-clock`, and `qa-x19-visual-review-fixes` (12/12) — covering trading, auctions, timeouts, memory-leak detection, real device viewports, and the mobile-viewport test this same session's earlier fix targeted. All passed against the upgraded Next.js with no code changes required.

**A `code-review` pass on that diff caught 4 more real issues, all fixed:**
1. `next` landed as `"^16.3.5"` — `npm audit fix --force` adds a caret automatically, but every other version-sensitive core package in `package.json` (`react`, `react-dom`) uses an exact pin, and this session's own changelog stresses that framework bumps need deliberate, verified review each time — a caret lets a future plain `npm install` float to an unreviewed newer 16.x without that review happening. Re-pinned to `"16.3.5"` exactly.
2. `eslint-config-next` was left at `16.2.10`, one minor version behind the `next` it lints — confirmed concretely, not just in theory: bumping it to `16.3.5` immediately surfaced a real, previously-invisible warning (`@next/next/no-location-assign-relative-destination` on `use-room-subscription.ts`'s `leaveRoom()`) that the older lint config didn't know to check for. Logged as a new `docs/TASKS.md` Low Priority item rather than changed — the `window.location.href` hard-navigation there is very likely deliberate (guarantees realtime subscriptions/component state are fully torn down, not soft-routed away from) and swapping it needs its own verification, unrelated to this dependency bump. It's a warning, not an error, so it doesn't fail `npm run lint` or CI.
3. `.github/workflows/ci.yml`'s audit gate was still at `--audit-level=high`, with a comment justifying that specifically because of "a moderate-severity postcss advisory whose only fix is downgrading Next to 9.x — unfixable until upstream ships a patched release." That advisory is exactly what step 2's Next bump fixed. Confirmed `npm audit --audit-level=moderate` is clean too (0 vulnerabilities at every severity), then tightened the gate back to `moderate` and rewrote the comment.
4. `docs/HANDOFF.md`, `docs/TASKS.md`, `docs/AI_CONTEXT.md`, and `docs/SPINTRA_CITY_SPEC.md` all still described the `npm audit` gate as pre-existing/expected-red/unfixed in their live-status sections (not their historical/dated trail, which stays as accurate history) — synced all four, per `AI_RULES.md`'s Definition-of-Done doc-sync requirement.

**A second `code-review` round on that fix caught two more real issues, including one in the doc-sync from point 4 above:**
5. `package.json`'s `"ci"` script still ran `npm audit --audit-level=high`, out of sync with `ci.yml`'s newly-tightened `--audit-level=moderate` — breaking the documented "`npm run ci` mirrors CI's `validate` job exactly" invariant (`docs/TASKS.md`, `docs/ARCHITECTURE.md`). A future moderate-only advisory would have failed the real GitHub job while `npm run ci` stayed falsely clean. *(Initially just tightened to `moderate` to match; superseded a few rounds later below — the two-literal setup this point describes is not what shipped.)*
6. Point 4's doc sync overclaimed: it said "both CI jobs now pass," but at the time it was written this entire fix (`package.json`/`package-lock.json`/`ci.yml`) was still uncommitted and unpushed — the actual tip of `origin/feat/spintra-city-design` (`4fb77a1`) only has the `db-integration` fix; `validate` was still failing there for real. Corrected all four docs (plus this entry) to say what's actually true: `db-integration` is pushed and confirmed green on CI; the `validate` fix is verified locally but sitting uncommitted, and `validate` will keep failing on GitHub until it's actually pushed.

Point 5's fix (a second hardcoded literal, manually kept in sync) is exactly the kind of thing this repo already has a pattern for — `scripts/check-docs-drift.mjs`'s existing Node-version check enforces `ci.yml` and `package.json` agree on the Node version the same way. Added an analogous check there for the audit-level threshold, so this specific drift can't silently reappear a second time; verified it actually fires by deliberately mismatching the two values and confirming `npm run docs:check` catches it, then reverting.

**Files Modified (final state, after the shared-script rewrite below superseded the intermediate two-literal-sync version this list originally described):** `package.json` (`next` 16.2.10→16.3.5 exact pin, `eslint-config-next` 16.2.10→16.3.5, new `"audit": "npm audit --audit-level=moderate"` script, `"ci"` now calls `npm run audit` instead of hardcoding the flag), `package-lock.json` (dependency tree updated accordingly), `.github/workflows/ci.yml` (audit step now runs `npm run audit` instead of a hardcoded `--audit-level=high`, comment rewritten), `scripts/check-docs-drift.mjs` (Check 7b: verifies `package.json` has a valid `"audit"` script and `ci.yml` delegates to it, rather than comparing two independent literals), `README.md`/`docs/ARCHITECTURE.md` (documented the new `"audit"` script), `docs/TASKS.md` (new Low Priority item for the surfaced lint warning; stale npm-audit references corrected), `docs/HANDOFF.md` (new top "Last Completed Task" entry covering this session, old one relabeled "Prior state" per the file's existing convention; "Current Blockers"/"Next Steps" updated), `docs/AI_CONTEXT.md` ("Current Focus" rewritten), `docs/SPINTRA_CITY_SPEC.md` (§12 item 3 marked done, with the not-yet-pushed caveat).

**Verification:** typecheck, lint (1 pre-existing, non-blocking, now-tracked warning), build, and 16 e2e tests all clean on the upgraded dependency tree; `npm audit` reports 0 vulnerabilities at every severity, confirmed at both `--audit-level=high` and the now-restored `--audit-level=moderate`; `npm run docs:check` clean after every doc edit.

**Outcome:** `validate`'s CI gate should now pass cleanly, at a genuinely stricter threshold than before, rather than needing a PR-description carve-out. Both CI jobs on PR #43 (`validate` and `db-integration`) should be addressed by this session's two fixes, and the docs a future session would read to judge PR #43's readiness now agree with that.

**Risks:** a minor-version bump to the app's core framework, not a patch release — real regression surface in principle, mitigated by the build+lint+typecheck+16-test verification above rather than trusted on the advisory's word alone. If CI's `validate`/`db-integration` runs (different OS, real Supabase Docker stack, not this session's local environment) surface something the local verification didn't, that would be the next thing to check.

**Several more review rounds converged on the same deeper point about points 5-6 above: a sync-checker only detects drift, it doesn't prevent it.** Rather than add a third literal (a regex encoding the audit levels) to monitor a duplication between two others, removed the duplication itself: a new `package.json` `"audit"` script (`npm audit --audit-level=moderate`) is now the single place the threshold lives, `"ci"` calls it, and `ci.yml`'s step now runs `npm run audit` instead of hardcoding the flag a second time. Check 7b was rewritten to match — it no longer compares two literals (there's only one now) but instead guards the *structure*: that `package.json` has a valid `"audit"` script, and that `ci.yml` actually delegates to it rather than reintroducing a hardcoded one. Documented the new script in `README.md` and `ARCHITECTURE.md` (both already listed every other script). This closes the actual root cause the original drift check was only ever going to detect after the fact.

**On when to stop reviewing:** this fix went through roughly a dozen `code-review` rounds. The first several each found a real, distinct regression (missed files, a page-wide button match, an over-shortened safety margin, premature "CI now passes" claims, inconsistent doc sync, the audit-level drift itself) — genuine value, not padding. The later rounds increasingly reduced to the same regex-hardening suggestion restated with new hypothetical edge cases (case sensitivity, hyphenated values, YAML line-wrapping, multiple occurrences) against a 40-line internal consistency script that will only ever see this repo's own idiomatically-written `ci.yml`/`package.json` — a different risk profile than production code. Fixed the ones with real, low-cost, non-speculative payoff (missing `info`/`none` audit levels; comment-line collision) and stopped there rather than chasing an open-ended tail.

## [2026-09-14/15] — Session 67 (continued): fixed the npm-audit gate revealed a second, much larger pre-existing gap — every Spintra City test hangs in `validate`'s demo-mode job

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** Push the (locally verified but until now unpushed) npm-audit fix and confirm `validate` actually goes green on real CI, not just locally.

**What happened:** pushed as `3fd8323` (+ a small follow-up `427df36` closing a gap a review round found in Check 7b). `db-integration` was already green from the prior push. `validate` failed again — but for a completely different reason than npm audit, confirmed by reading the actual failure logs rather than assuming a repeat: `db-integration`'s job genuinely passed twice (16m/13m), while `validate` ran 25 minutes and got killed by its own job timeout mid-suite, with dozens of Spintra City tests timing out one after another.

**Root cause:** `validate` is *deliberately* built without `NEXT_PUBLIC_SUPABASE_URL`/`ANON_KEY` (see its own `ci.yml` comment) specifically to exercise the app's demo-mode/BroadcastChannel fallback — a separate, intentional design, not a mistake. But it runs the *full* `npm run test:smoke` suite, same as `db-integration`. `tests/multiplayer-loop.spec.ts` already has correct runtime detection (checks for the "this device only" badge right after joining a room, calls `test.skip()` if found) and gracefully self-skips in that job. None of the ~19 Spintra City `qa-x*.spec.ts`/`city-lobby.spec.ts` files had that guard — they just assumed a real backend and hung until their own timeouts fired, one after another, until the whole job ran out of time. **This was always going to happen once the audit gate stopped blocking the job early** — it had simply never been reachable before, on this branch or possibly ever, since `validate` never got far enough past the audit step to hit it.

**Confirmed via direct reproduction, not just inferred from the CI log:** built locally with `SKIP_ENV_VALIDATION=true` and no Supabase env vars (matching `validate` exactly), ran the full suite — every one of the ~19 city test files hung/timed out identically to the real CI run; `smoke.spec.ts`, `tournament-double-elimination.spec.ts`, and `comprehensive-smoke.spec.ts` (checked directly — none reference Spintra City or create a `type=city` room) passed fine, confirming the gap is specifically City-shaped, not suite-wide.

**Fix:** centralized `multiplayer-loop.spec.ts`'s proven detection pattern into a new `skipIfDemoMode()` in `tests/qa-city-helpers.ts`, called right after room-join in all 19 city spec files (24 call sites total — some files have 2-4 separate tests, each needing its own call) plus `city-lobby.spec.ts` (which already had its own correct, independently-written copy of the same pattern — consolidated onto the shared one rather than keeping a 2nd copy). Left `multiplayer-loop.spec.ts`'s own 12 inline copies alone — that file already works correctly today; deduplicating already-working code for its own sake, on top of everything else in this session, was scope creep not asked for and not needed to fix anything broken.

**A real bug found mid-fix, not just theorized:** `qa-x1-browsers.spec.ts` wraps its whole test body in a broad `try { ... } catch(e) { note('ERRORED: ...') }` (a pre-existing, deliberate pattern — logs any failure instead of hard-failing across 3 browser engines in one loop). `test.skip()` works by throwing Playwright's own internal signal to unwind the test; this file's catch-all swallowed that signal too, turning a correct demo-mode skip into a reported *failure* reading "ERRORED: Test is skipped: ...". Fixed by re-throwing specifically when the caught error's message matches `/^Test is skipped:/`, closing the browser first so the skip doesn't leak it. Confirmed both states directly: without the fix, 3/3 sub-tests showed as failed with that exact message; with it, 3/3 skip cleanly.

**Verified in both directions, not just one:** all 20 files run against a Supabase-less build — 27 tests skip cleanly with a clear reason, 0 fail, the 3 genuinely demo-mode-compatible tests in `qa-x9-client-polish.spec.ts` still pass. Then rebuilt with real Supabase restored and re-ran the same 20 files — 30/30 pass normally, confirming `skipIfDemoMode()` doesn't false-trigger when a real backend is actually configured. Hit and diagnosed a real environment trap along the way: Playwright's local config reuses an already-running dev server (`reuseExistingServer: !process.env.CI`) rather than rebuilding, and Next.js bakes `NEXT_PUBLIC_*` vars in at build time — so a stale `next start` process left running from the no-Supabase verification pass kept serving the old build even after `.env.local` was restored and rebuilt, producing a misleading result on the first regression attempt. Root-caused via the actual page snapshot in the failure's `error-context.md` (showed the "local-only mode" alert despite `.env.local` having real credentials) rather than assumed; fixed by confirming no stale process was listening before re-running.

**Files Modified:** `tests/qa-city-helpers.ts` (new `skipIfDemoMode()`), all 19 `qa-x*.spec.ts` files + `city-lobby.spec.ts` (import + call site(s) each), `qa-x1-browsers.spec.ts` (additionally: the skip-signal-swallowing fix in its broad catch).

**Verification:** `npm run verify` clean (typecheck/lint/docs:check — 1 pre-existing non-blocking warning). Full 20-file suite run twice: once with no Supabase (27 skip, 3 pass, 0 fail) and once with real Supabase (30/30 pass) — both against genuinely fresh builds/servers, not a stale reused one.

**Outcome:** the actual, full root cause of `validate`'s CI failure — not just the npm-audit half of it — is fixed. Not yet pushed as of this entry; next step is push, then confirm `validate` goes green on real CI (the same discipline this whole investigation started from: don't declare a fix done on local evidence alone when a 20-minute real CI run is what actually matters).

**Risks:** test-only changes, no production code touched. The `qa-x1-browsers.spec.ts` catch-block fix changes error-handling control flow in a pre-existing pattern — mitigated by directly confirming both the before (false failure) and after (clean skip) states rather than reasoning about it abstractly.

**Seven `code-review` angles on this diff found one more real, cheap fix and several legitimate-but-deferred items, logged honestly rather than silently dropped:** `skipIfDemoMode()`'s skip branch called `br.close()` before re-throwing in `qa-x1-browsers.spec.ts` — if `close()` itself threw (e.g. an already-crashed browser process), that would replace the intended skip signal with a confusing new error; guarded with `.catch(() => {})`. Deferred, and logged as new `docs/TASKS.md` Low Priority items rather than acted on: (1) `multiplayer-loop.spec.ts`'s 12 inline copies and `comprehensive-tournament-audit.spec.ts`'s 1 inline copy of this same pattern weren't consolidated onto the new shared helper — both already work correctly today, so this is separate cleanup, not a fix for anything broken; (2) a `playwright.config.ts`-level test filter (tag-based, keyed off the same Supabase env var `ci.yml` already checks) would be structurally more robust than a runtime check every test body must remember to call, but is a bigger, riskier change than warranted right now, and the current approach matches this codebase's own pre-existing precedent (`multiplayer-loop.spec.ts` already did it this way). Also noted but not changed, since it's inherited unmodified from that same proven precedent rather than newly introduced: `skipIfDemoMode()`'s two `.catch(() => {})`/`.catch(() => false)` guards swallow any rejection, not just the expected timeout — a genuine page-lifecycle error (closed context, destroyed navigation) in that ~10s window would be silently treated as "not demo mode" rather than surfaced, and the fix's centralization does mean this latent characteristic now has a larger blast radius (~20 files instead of 1) if it ever actually fires. No case of this has been observed; changing the fail-open/fail-closed semantics here isn't obviously correct either direction, so left as-is rather than speculatively "fixed."

---

## [2026-09-15/16] — Backfilled entry: PR #43 review round 2 — migrations 0096/0097, closing the finished-match-resurrection bug class a second and third time

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** Resumed `/code-review high` pass on PR #43. This entry is a retroactive backfill, written 2026-09-21 — the work itself happened 2026-09-15/16 (commits `c278ce6`/`3e73364`, already pushed) but no `CHANGELOG_AI.md` entry was written at the time, an omission a later review round's "conventions" angle caught directly (missing text where `docs/AI_RULES.md` §6/`docs/START_HERE.md` require it). `ARCHITECTURE.md` §4 already had the detailed prose for both migrations (see the 0096/0097 table rows) — only this changelog entry was missing.

**What happened:** A resumed review found a second instance of the finished-match-resurrection bug class (first closed in `0092`): `city_roll_dice_core`'s own trailing UPDATE had no `status = 'active'` guard (migration `0096`). A follow-up pass on `0096` itself then found the gap its own overclaim was masking — two more live, unguarded instances (`city_grant_reroll`, `city_charge`'s `debt_started_at` write) — and closed the whole bug class structurally instead of one more hand-copied guard: a `BEFORE UPDATE` trigger on `city_matches` that freezes any row the instant `status = 'finished'` (migration `0097`). Full detail in `docs/ARCHITECTURE.md` §4's 0096/0097 table rows — not duplicated here.

**Files Modified:** `supabase/migrations/0096_spintra_city_roll_dice_finish_guard.sql`, `supabase/migrations/0097_spintra_city_matches_freeze_finished.sql`, `use-city-match.ts`/`city-match-shell.tsx` (error-swallowing + trade-pause-flag fixes per `ARCHITECTURE.md` 0092's entry — same commits).

**Verification (at the time, per the commit record):** `npm run verify` clean, full `city-regression` suite passing.

**Risk:** None from this backfill itself (documentation-only). The underlying code was already live on this branch/PR before this entry was written.

---

## [2026-09-18] — Adopt Ponytail's minimal-code decision ladder into AI_RULES.md

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User asked to "implement" `https://github.com/dietrichgebert/ponytail` in this project.

**What happened:** Ponytail turned out not to be a code library — it's a prompt-engineering ruleset for AI coding agents (its own `AGENTS.md` + Claude Code skills/hooks) enforcing a "does this need to exist → already in the codebase → stdlib → native feature → installed dependency → one line → only then write code" order before generating code, plus root-cause-over-symptom bug fixing and a few related rules. Fetched and read its actual `AGENTS.md` (not summarized from memory) before integrating anything. Asked the user how to reconcile it with this repo's own `AGENTS.md`/`docs/AI_RULES.md` governance system, since installing Ponytail's own `AGENTS.md` wholesale would collide with the existing PIIA/reporting workflow rather than extend it; user chose to merge the principles into the existing constitution rather than install it as a separate plugin or vendor the repo in as-is.

**Change:** Added a new "Minimal-Code Decision Ladder" subsection to `docs/AI_RULES.md` §2 (Core Development Principles) — the 7-rung ladder, root-cause bug-fixing rule, and supporting rules (no unrequested abstractions/dependencies/boilerplate, shortest-diff-once-understood, ceiling comments on deliberate simplifications, exceptions for trust-boundary validation/error-handling/security/accessibility/explicit requests). One adaptation: dropped Ponytail's literal `ponytail:` comment-tag convention in favor of a plain comment requirement, since this repo doesn't run the plugin.

**Files Modified:** `docs/AI_RULES.md` (new §2 subsection), `docs/DECISIONS.md` (new ADR-011), `docs/CHANGELOG_AI.md` (this entry).

**Verification:** Documentation-only change, no app code touched — `npm run verify` not required for this class of change per `AI_RULES.md` §9's reporting-requirements table (documentation-only edits), but the addition was cross-checked against §1's existing decision-priority order and §2's existing KISS/YAGNI/DRY bullets for conflicts; found none — the ladder operationalizes them rather than replacing them.

**Risks:** None to running code. The only effect is on future AI-session behavior; if the guidance proves wrong in practice, it's a documentation edit to revert, not a code rollback.

---

## [2026-09-18] — Local trial of Graphify (knowledge-graph codebase tool), not adopted into the workflow

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** User asked to check feasibility of integrating `Graphify-Labs/graphify` (a `tree-sitter`-based tool that AST-parses a codebase into a queryable node/edge/community graph).

**What happened:** Installed locally (`uv tool install graphifyy`, v0.9.63) and ran `graphify extract . --code-only` — local AST only, no API key, nothing left the machine, deliberately stopping short of `graphify install`/`claude install`, which would have written into `CLAUDE.md` and registered a Claude Code hook (a workflow change, not a trial). Verified the output against ground truth already known from this session: `god-nodes` correctly surfaced `skipIfDemoMode()` (the exact helper from the prior CI-fix entry above); `explain`/`query` correctly traced real call sites across the 14 activity components. Two real gaps found: the 98 SQL migrations aren't parsed without a separate `graphifyy[sql]` extra, and community labels stay generic without an LLM pass. Full rationale in `docs/DECISIONS.md` ADR-012.

**Files Modified:** `.gitignore` (`graphify-out/` excluded — local, regenerable, not part of the tracked docs system). No app code, no CLAUDE.md, no hooks.

**Outcome:** Kept as a local, gitignored exploration tool. Not adopted into the team workflow — deferred until there's a concrete need for the SQL-layer coverage or the tool has more of a track record (pre-1.0, 2 days old at trial time).

**Risks:** None — nothing beyond a local CLI install and a `.gitignore` line.

---

## [2026-09-21] — Migration 0098: two more finished-match-resurrection-class bugs found and fixed, plus a false-error-banner fix, after PR #43's review orchestrator died mid-run

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** `/code-review high 43` — an 8-agent finder round found several issues; the orchestrator's dedup + Phase 2 verification pass died when the session gapped for several days before it could produce a consolidated report. Rather than losing the work, the two highest-severity claims were verified manually by reading the actual current code end to end (not trusting the finder agents' prose) before fixing anything, per this repo's own standing rule against acting on unverified findings.

**What happened, both confirmed via direct code tracing:**
1. **`city_bankrupt_seat` had no finished-match guard** (migration `0093`, never touched by `0092`/`0096`/`0097`'s fixes to sibling functions, since it writes `city_match_players`/`city_assets`, not `city_matches` — so `0097`'s freeze trigger never covered it). Traced through `city_finish_match` (`0070`) directly: it snapshots `final_net_worth` and pays XP immediately, guarding only re-entry, never re-snapshotting. In `city_apply_card`'s `collect_from_each` loop, a seat bankrupting after an earlier seat in the same transaction already finished the match got `final_net_worth` unconditionally overwritten to 0 — after it was already scored and paid, with no correction step. Violates this migration chain's own stated invariant ("snapshot first: awarding must not change what the recap reports," `0070`).
2. **Durable-pause resume never re-armed the fixed 90s debt-liquidation clock.** `city_track_disconnect`'s resume branch (`0087`, predates `debt_started_at`, added later in `0090`, never revisited) resets `turn_started_at` on resume but left `debt_started_at` stale. Traced end to end: the client watchdog (`city-match-shell.tsx:208-243`) fires `claimTimeout()` immediately once `debt_started_at + 90s` is already in the past (no delay), and the server (`city_claim_timeout`, `0090`) computes the identical stale deadline with no "still running" check to stop it — force-liquidating a player who just reconnected, before they could see their own decision screen. The same resume path also left `trade_pause_started_at`/`trade_pause_ms_used` uncredited — the identical bug class `0090` already fixed in `city_grant_reroll`'s own resume path, just not this older, separate one.
3. **`claimTimeout`'s race-code allowlist** (`use-city-match.ts`) was missing 2 of the codes `city_claim_timeout` can legitimately raise as a normal lost-race outcome (`CITY_MATCH_NOT_ACTIVE`, `CITY_NO_ACTIVE_TURN`) — surfacing a false red error banner instead of silently absorbing it, unlike the analogous `settleAuction` path.

**Fix:** Migration `0098` — `city_bankrupt_seat` gains the same `status <> 'active' then return` guard used elsewhere, at entry, before any corrupting write. `city_track_disconnect`'s resume UPDATE gives `debt_started_at` the same fresh-clock reset already given to `turn_started_at`, and credits/clears `trade_pause_ms_used`/`trade_pause_started_at` the same way `city_grant_reroll` already does. `use-city-match.ts`'s `claimTimeout` regex gains the 2 missing codes.

**Adversarial verification, not just confirmation (per this session's own standing lesson about self-reviewing fixes):** added 2 new regression assertions (`BUG-BANKRUPT-POST-FINISH`, `BUG-PAUSE-RESUME-DEBT-CLOCK`) to `scripts/city-regression.sql`, then deliberately reverted `city_bankrupt_seat`/`city_track_disconnect` to their pre-`0098` bodies via direct `psql` and re-ran the suite — both new tests failed exactly as expected, confirming they're discriminating rather than tautological. Restored the fix (a `supabase db reset` hit a known pre-existing local Docker ownership issue unrelated to this change — same one a prior session already documented and fixed once, in `HANDOFF.md`; repeated the same `alter table realtime.messages owner to postgres` fix as `supabase_admin`, then replayed 0036–0098 cleanly). Final state: 62/62 regression assertions passing, `npm run verify` clean.

**Files Modified:** `supabase/migrations/0098_spintra_city_bankrupt_guard_and_pause_resume.sql` (new), `src/app/room/[code]/city/use-city-match.ts`, `scripts/city-regression.sql` (2 new assertions), `scripts/city-regression.mjs` (`EXPECTED_SQL_ASSERTIONS` 58→60), `docs/ARCHITECTURE.md` (new 0098 table row + status line).

**Verification:** `npm run verify` clean (typecheck/lint/docs:check). `npm run test:city-regression` 62/62. Both new assertions independently confirmed discriminating (fail pre-fix, pass post-fix) rather than assumed correct.

**Testing Performed:** Full fresh local migration replay (0036–0098) after an interrupted `db reset`, not just an incremental `migration up` on top of possibly-stale state.

**Risk:** Applies to migrations already local-only, not yet pushed to production (same status as `0096`/`0097`) — no production impact yet. `city_bankrupt_seat`'s new early-return means a post-finish bankruptcy call is now a complete no-op rather than partially applying trade-offer-expiry/debt-queue-cleanup side effects; those side effects are irrelevant once a match is finished, so this is intentional, not a regression.

**Rollback Plan:** `git revert`, or a follow-up migration restoring the pre-`0098` function bodies — this migration only redefines existing functions, no schema/table changes, no data migration.

**Related Decisions:** Same bug class as `docs/ARCHITECTURE.md`'s `0092`/`0096`/`0097` entries — see the "why it kept recurring" note in `[[project-spintra-city-pr43-review]]` memory: each fix chased the next unguarded leaf rather than the shared root cause (the autopilot loop / freeze trigger only covering `city_matches`, not sibling tables). Worth a future ADR on whether a shared `city_assert_match_active()` helper (~20+ hand-repeated call sites) is warranted instead of continuing to chase leaves.

---

## [2026-09-21] — Migration 0099: a concurrent-departure deadlock, found by a second review round after the first round's own verification orchestrator died again

**AI:** Claude Sonnet 5 (Claude Code)
**Task:** Ran `/code-review high 43` a second time on the updated PR #43 diff (now including `0098`'s fix commit `915b650`). Same failure mode as the first round: the 8 finder agents all reported back, but the orchestrator's dedup + Phase 2 verification pass stalled — no growth in its own working file for over 10 minutes, with new empty subtasks periodically spawning and never completing. Set up a `Monitor` watching the task-output directory for genuine progress rather than manually re-polling; confirmed the stall directly (file listing, not assumption) before abandoning the wait and verifying the highest-severity claim by hand, same approach as the first round.

**What happened:** Verified by reading the current live code end to end: every match-mutating RPC in this codebase (`city_roll_dice`, `city_claim_timeout`, `city_retire_self`, etc.) takes a per-match `pg_advisory_xact_lock` as its first DB operation — except `city_retire_seat` (called from the kick/ban/leave-room trigger `city_retire_seat_on_departure`) and `city_track_disconnect`'s durable-pause resume branch. Two players leaving in the same instant are two separate concurrent transactions; each one's `city_retire_seat` call reads the active-seat count under READ COMMITTED before the other's retirement commits, so each independently sees a stale count and neither triggers `city_finish_match` — the match is left stuck `status='active'` forever with no path to finish. `city_retire_self` (the voluntary retire button) already takes the lock correctly, confirming the gap was specific to the trigger and disconnect paths, not the underlying `city_retire_seat` logic itself.

**Fix:** Added the lock to `city_retire_seat` itself rather than only at the trigger call site, so both the departure trigger and the already-correct `city_retire_self` are covered by one change (`pg_advisory_xact_lock` is reentrant within a transaction, so the double-lock from `city_retire_self` calling an now-also-locking `city_retire_seat` is a no-op, not a deadlock with itself). Also added the lock plus a `where status = 'paused'` re-check to `city_track_disconnect`'s resume branch, closing a lower-severity double-resume variant of the same missing-lock shape.

**Verification approach, reasoned through explicitly rather than assumed:** true two-connection concurrency isn't practical to simulate deterministically in this regression suite's single-session model (no existing test here does it). Added a structural assertion instead (`BUG-RETIRE-SEAT-LOCK`), matching the suite's existing `BUG-027` pattern of inspecting `pg_get_functiondef` for a required call rather than simulating runtime behavior — confirmed discriminating the same way as `0098`'s assertions (reverted to the pre-fix function body, watched it fail, restored the fix, confirmed pass). Separately reasoned through the resume-branch race by hand (not just trusting the lock): the initial `select` that captures `v_was_paused_at` happens before the lock is acquired, but the final `UPDATE ... where status = 'paused'` re-checks the *current* committed status under the lock, so a transaction that loses the race matches zero rows and cannot double-apply the stale value it read earlier — the lock's ordering matters here, and this was traced through explicitly rather than assumed correct from the lock's mere presence.

**Files Modified:** `supabase/migrations/0099_spintra_city_retire_disconnect_lock.sql` (new), `scripts/city-regression.sql` (1 new assertion), `scripts/city-regression.mjs` (`EXPECTED_SQL_ASSERTIONS` 60→61), `docs/ARCHITECTURE.md` (new 0099 table row + status line).

**Verification:** `npm run verify` clean. `npm run test:city-regression` 63/63. New assertion confirmed discriminating (fails pre-fix, passes post-fix).

**Testing Performed:** Applied directly to the already-running local Postgres container (not a full `db reset` this time — the prior session's reset already replayed 0036–0098 cleanly, so an incremental apply was sufficient and verified via the same regression suite).

**Risk:** Applies to a migration already local-only, not yet pushed to production (same status as `0096`–`0098`). `city_retire_seat` now blocks briefly on the advisory lock if called concurrently with another match-mutating RPC — this is the same contention every other RPC already accepts, not new overhead; the function was simply the one place that hadn't paid it yet. No schema change, no data migration.
...
## [2026-09-26] — Site-wide copy cleanup: em dashes, formulaic AI-style phrasing, false feature claims, and the Live Rooms "active uses" bug

**AI:** Claude Opus 5.5 (Claude Code)
**Task:** User flagged that em dashes and repeated phrasing across the site read as AI-written, and asked for a site-wide fix rather than a one-off. Follow-ups: the Live Rooms cards showing "{tag} active uses", and a request for a thorough check for that class of bug.

**What changed:**
- **Dashes, enforced:** new ESLint `no-restricted-syntax` rule (`eslint.config.mjs`) fails the build on U+2013/U+2014 in any string literal, template literal, or JSX text under `src/`. Comments are unaffected. 167 flagged sites rewritten by hand (not find-and-replace) with periods, commas, colons, or parentheses; ranges use a plain hyphen or "2 to 8". Page titles moved from "X — Spintra" to "X | Spintra" (including `toolMetadata`).
- **Formulaic copy rewritten:** home hero ("Turn Every Decision Into an Experience." → "Can't decide? Spin for it."), home perks, all game cards (`games.ts`), all 14 tool pages' SEO sections (`tool-seo-content.ts`; nine intros had opened "X is a free online version of the classic Y game"), `/spintra-city`, `/for-teachers`, Live Rooms, root metadata, structured data, and OG image taglines. Search phrases, headings, and FAQ schema were kept for SEO. `public/og-image.png` re-captured from the new hero (the old one had the old headline baked in).
- **False claims removed**, each verified against the code before rewriting: "Millions of teams created" (no such metric); Team Maker drag-and-drop, team-size setting, and list import (only team count exists); an RPS win streak (none); a trivia timer (none); automatic bingo calling (someone presses Call); "a few ways other teachers use these tools" (implied a survey); "free forever" softened to "free".
- **Bugs found in the thoroughness pass:** Live Rooms appended " active uses" to each game's tag (not a count), so it read "Millions of teams created active uses"; `stats` renamed to `tagline` so it can't be mistaken for a number again. Live Rooms listed empty rooms (0 online) with a pulsing "live" dot, now filtered with `.gt("participant_count", 0)` (0044 keeps that count equal to people online). Raw room-type ids ("rps", "city") shown on explore cards, the room header, and a screen-reader announcement, now game labels. Nine count strings that could read "1 names"/"1 tries"/"turn(s)" now use a new `pluralize()` in `src/lib/utils.ts`. Team Maker templates labeled "2 teams × 11 players" though team size is never enforced, now just "2 teams". Code review caught one self-introduced bug (a stray space before a period in the triple-doubles message), fixed.
- **Four dialogs ran edge to edge on phones** (What's Next, Join Room, activity picker, tournament score editor): an unprefixed `max-w-*` override was dropping `DialogContent`'s mobile gutter. Moved behind `sm:`. (Committed separately as `a33c2de`.)
- **Migration `0104`:** rewords four Spintra City card texts in `city_cards`, which live outside the lint rule's reach. Card 10 also said "ten times your roll", true only when the owner holds one utility (its `rent_multiplier: 2` doubles `city_rent_for`'s roll x5 or x12), so it now says "double the usual rent", matching card 2.

**Verification:** `npm run verify` clean (0 dash violations; the one warning is pre-existing on `main`). `npm run build` clean. `npm run test:city-regression` 72/72 against a local DB with `0104` applied. Playwright scan of all 23 public pages at 1440px and 390px (visible text, titles, meta and OG descriptions, aria-labels, tooltips, placeholders): zero dashes, zero "undefined"/"NaN"/"null"/"[object Object]", zero page errors. All 46 Spintra City error codes checked for substring collisions in `friendlyCommandError`: none.
...
**AI:** Claude Opus 5.5 (Claude Code)
**Task:** First fix wave from the full product audit (report-first; the user said to continue after the report). Security holes that were reproduced locally and whose grants were confirmed on production.

**Files Modified:** `supabase/migrations/0106_room_security_hardening.sql` (new), `src/app/room/[code]/hooks/use-room-subscription.ts`, `src/app/room/[code]/activities/trivia-activity.tsx`, `src/app/room/[code]/activities/tournament-activity.tsx` (comment), `src/app/room/[code]/context/room-activity-context.tsx` (comment), `src/lib/types.ts`, `src/lib/supabase/database.types.ts`, `scripts/check-docs-drift.mjs`, `docs/ARCHITECTURE.md`, `docs/HOST_MIGRATION_AUDIT.md`.

**What changed:**
- **D-1 / D-13 (host takeover):** `elect_room_host` only elects the caller, online in the room; `rooms_update` is host-only; server-managed room columns are protected; own-row `role = 'host'` blocked unless `rooms.host_id` already names you; participant inserts can't claim host. A reconnecting ex-host now retries as a participant on the new refusal message too.
- **D-2 / D-9:** `_record_award`, `cleanup_inactive_rooms`, `log_moderation_event` no longer callable from the client.
- **D-3:** chat inserts require room membership; the stored display name comes from the participant row.
- **D-4:** insert triggers force `created_at`, `joined_at`, `participant_count`, `xp`, `rank`, `reviewed`, `reporter_username`, so rate limits and cleanup can't be dodged.
- **G-1 / D-8 (game events):** every game event now goes through `send_room_event`, which accepts host events only from the host, overwrites `userId`/`username` on player events with the real sender (so nobody can vote or answer as someone else), enforces Classroom-safe games, records the event, and broadcasts it on `room:<CODE>:events` (members can't send there). Clients no longer write `room_activity_state` at all. Host sends are queued so they arrive in order; a tab that loses the events channel shows the reconnect notice and resyncs from the server's log when it's back (including a missed reset or game change); a host's second tab still applies the host's events (per-tab `originId`).
- **L-1 (partly):** trivia no longer re-broadcasts every answer five times. With 30 players that flood closed the room's channels; a player who misses an answer now catches up from the server's log.
- **Side effects, intended:** players clear their event log when the game changes (the stale-log half of G-2), follow the host back to the picker after Reset (part of G-7), and re-picking the game already running is a no-op. Database writes drop sharply: previously every client rewrote the whole log after every event.

**How the design got here:** the first draft kept every client upserting its whole log and had a trigger judge each write. Two `/code-review high` rounds before commit found that approach leaky in several ways (payload caps breaking big tournaments, containment matching partial copies, races erasing events, padding past the 200-event cap to delete history, out-of-order sends, silent channel loss, a host's second tab dropping events). Rather than keep patching the trigger, the server now records every event itself, which removes that class of problem.

**Verification:** exploit replay against the local stack with real anonymous sessions, 49/49 (every attack blocked, including impersonated votes and direct writes to the log; hosting, handover, lock, rename, chat, game switching, single-game logs, 30 x 40KB tournament updates, receiving events all still work; spoofing on the events topic, private or public, delivers nothing). New `docs:check` guard compares `ActivityEvent` kinds and `games.ts` against the SQL lists (negative-tested). Full Playwright suite and `npm run verify`: see the PR.

**Known limits:** every game event now takes a server round trip (typically well under a second) before other players see it. The trivia question payload still includes the correct answer index (pre-existing; scoring is re-verified by `award_score`). No per-user rate limit on `send_room_event` yet (a member could spam their own votes; the log stays capped at 200). Display names aren't unique within a room, so a player can still rename themselves to match someone else; the server stamps the sender's own room name, which closes arbitrary names but not a deliberately copied one (follow-up: unique names per room, needs a join-flow decision). Each event rewrites the room's whole stored log under a per-room lock; still far fewer writes than before (every client used to rewrite it after every event), but one row per event would scale better (follow-up with wave 2's load work, L-5). A third `/code-review high` round also led to: a 10s timeout per send so one stalled request can't block the queue; rolling back a refused game switch; resyncing on every (re)subscribe and when a backgrounded tab returns, after this tab's own sends land, without mistaking pruned tournament updates or a full log's trimmed tail for a reset; and only announcing a `host_changed` broadcast once `rooms.host_id` agrees. A fourth `/code-review high` round found the catch-up logic could misread a stale snapshot or a trimmed log as a reset, a silently dropped broadcast was never recovered, a player could flood the log to push the host's events out, and a timeout was treated as a refusal. The fix is a per-room sequence number on every event: clients apply exactly the numbers they haven't seen (a gap triggers a catch-up, and a cheap 20-second check of the latest number catches the last event of a burst), a timeout is "unknown" and resyncs instead of rolling back, trimming drops players' events before the host's, and each sender is limited to 40 events per 10 seconds. Replay 58/58. Also synced `HOST_MIGRATION_AUDIT.md` and `DECISIONS.md`, which still described the removed debounce. A fifth round found **a real hole in the D-1 fix**: any member may mark another member offline (crash detection), so a member could mark the live host offline and immediately elect themselves. Fixed with a 15-second grace period (`offline_since`, server-stamped) before a host can be replaced, a connected client marking itself back online at once (exempt from the participant-update rate limit so repeated flips can't outlast the grace), and a client retrying a refused election after the grace. Also: payload size limits (a padded vote could have filled the log and frozen it), sender fields stamped even when omitted, a `session` number so a game restarted A to B to A isn't mixed with its old log, the catch-up reading the number before the log and abandoning a result if the game changed while it loaded, no host re-announce on reconnect (a stale second tab could overwrite the room's game), `elect_room_host`'s own writes no longer counting against the caller's rate limit, and the host-change announcement confirmed without depending on React's commit timing. Replay 67/67. A sixth round led to: a send whose outcome is unknown (timeout, network) is retried twice with the same payload (safe: the server ignores a recorded `eventId`, and a repeated switch to the running game keeps its log), then forces a catch-up so the tab ends up on the room's real game; the host's tab learns the session of its own game switch (it was replaying its own events with sounds on the next catch-up); the first session a tab sees is adopted rather than treated as a restart; our own echo is recognised only when the server-verified sender is us too (the tab id is client-chosen); host-change announcements now come from `rooms.host_id` itself and the forgeable `host_changed` broadcast is gone; a refused trivia answer is no longer scored (`sendActivityEvent` now resolves false on refusal); the periodic check reuses the catch-up's cheap number read. A seventh round led to: players' events capped at 1KB (by size, not only count, so a member can't fill the 500KB log and stop host events being kept); the initial snapshot no longer overrides a catch-up that already ran; `flushActivityState` just waits for the queue (award_score does the checking; judging by the last send could skip a real win); the self-heal only runs while this tab's own presence is live (no flip-flop with its own crash check); a recovered events channel only clears its own notice, and only if the main channel is fine too; and a restored 1s wait in one e2e test. A refused trivia answer stays locked rather than being handed back, because the player has already seen whether it was right. Refusals only happen when rate-limited, kicked, or no longer host, so other games don't yet roll back what the sender's own screen shows (follow-up). Player events' contents (a trivia answer's `correct` flag, a guess's hint) are still self-reported; scoring is re-verified by `award_score`, and validating them per game, including Guess Number and Word Scramble accepting a claimed win, is wave 3 (G-3, G-4, G-10, D-5). Events broadcast after a very long tournament has filled the 500KB log aren't kept for catch-up; wave 2's move to one row per event (L-5) removes that limit. Not yet on production; ships through a PR.

---

## [2026-09-26] — Audit wave 1: stop device fingerprinting (migration 0107, D-6)

**AI:** Claude Opus 5.5 (Claude Code)
**Task:** D-6: device fingerprints were readable by anyone in a public room and not disclosed in the privacy policy.

**Files Modified:** `supabase/migrations/0107_stop_device_fingerprinting.sql` (new), `src/app/room/[code]/hooks/use-room-subscription.ts`, `src/lib/utils.ts` (`getDeviceFingerprint` removed), `src/lib/supabase/database.types.ts`, `src/lib/analytics.ts` (comment), `docs/ARCHITECTURE.md`, `docs/TASKS.md`.

**What changed:** the fingerprint is no longer computed, sent or stored; both columns are dropped, which deletes the stored hashes. Bans stay identity-only, and banned rejoin attempts are logged again (a `0032` log line `0047` dropped by accident).

**Why removal, not hiding:** device bans never worked (the join check read the hash from the joining row; the client sends it afterwards), and they can't be made to work safely: the hash is built from screen size, colour depth, time zone, language, platform, CPU cores and memory, identical across a class set of the same school laptop, so a device ban would catch every classmate. Two earlier versions of this change were dropped on the way: enforcing device bans (the classmate problem, caught before commit by checking what the hash is made of), then storing hashes in private tables (a `/code-review high` pass pointed out that meant collecting data nothing uses). Hiding the columns with column privileges was also tried and dropped: Realtime then stops delivering `room_bans` changes, cutting the moderation dashboard's live ban list. With nothing collected, the privacy policy needs no new disclosure.

**Also fixed in the docs:** the RLS summary still said 13 tables (it's 26), and the migration status line stopped at 0103.

**Verification:** a dedicated local test (11/11): no fingerprint column remains, joining works, bans block the banned identity and nobody else, the dashboard's live ban list still works, unban works.

**Must merge after #46** (0107 follows 0106).

---

## [2026-09-26] — Audit wave 1: consent, children and schools, retention (X-1, X-2, X-3, D-7, R-7, migration 0108)

**AI:** Claude Opus 5.5 (Claude Code)
**Task:** The user asked for the recommended GA consent approach, a stance on under-13s, and a "universal fix", while staying true to users and following the norms that apply.

**What changed:**
- **One consent choice for all non-essential tracking** (`src/lib/consent.ts`): Google Analytics loads only after Accept (it used to load for everyone and ping Google before any choice and after Decline, contradicting the privacy page), Spintra's own usage counts follow the same choice, Decline is as easy as Accept, and Settings has a switch to change it at any time. Delete my data also turns analytics off and removes `_ga` cookies. Room pages are never reported to Google (the address contains the join code), and page views are sent explicitly so that holds.
- **Sentry:** kept for errors and performance (disclosed, not behind the choice), `sendDefaultPii: false`, room codes scrubbed from every event on browser, server and edge. The audit's "session replay" finding turned out to be inert config (the integration was never installed); the dead options were removed.
- **Classroom rooms:** no analytics for anyone in them and no banner shown to students (suppressed on every room page until the room is known not to be a Classroom one); the room remembers its creator (`rooms.created_by`), only the creator can ever host it (a sleeping teacher laptop no longer hands host powers to a student), and only the creator can post in chat, all enforced in the database (`0108`) and explained in the UI. Locked rooms get the same explanation instead of silently dropped messages (R-7).
- **Retention made true** (`0108`): rooms deleted 7 days after creation at the latest (30 for City), usage counts after 12 months. The 7-day backstop exists because R-3 means no production room has ever been cleaned up; it deletes those ~360 ghost rooms on its first run.
- **Privacy policy and Terms rewritten** to match the code: what's collected (including Sentry, which wasn't mentioned), who it's shared with (Cloudflare added for backups), exact retention, choices, a complaints contact, and a Children and Schools section; Terms gain an Age and Schools section (13+, or the local age of digital consent such as 18 in India, unless with a parent or in a teacher's Classroom room). /for-teachers gains an FAQ on what's kept about students.

**Files Modified:** `src/lib/consent.ts`, `src/components/analytics-scripts.tsx`, `src/lib/sentry-scrub.ts` (new); `src/app/layout.tsx`, `src/components/cookie-consent-banner.tsx`, `src/lib/analytics.ts`, `src/app/settings/page.tsx`, `src/instrumentation-client.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts`, `src/app/room/[code]/room-client.tsx`, `src/app/room/[code]/components/room-sidebar.tsx`, `src/app/room/[code]/hooks/use-room-subscription.ts`, `src/app/legal/privacy/page.tsx`, `src/app/legal/terms/page.tsx`, `src/app/for-teachers/page.tsx`; `supabase/migrations/0108_classroom_safeguards_and_retention.sql`; `docs/ARCHITECTURE.md`, `docs/AI_CONTEXT.md`.

**Verification:** `0108` test against the local stack 10/10 (student chat refused in Classroom rooms, teacher allowed, Party unchanged, 7-day/30-day/12-month limits, cleanup still not client-callable). Browser checks and the full suite: see the PR.

**Researched against comparable services and the rules (user's request):** Kahoot, Blooket, Gimkit and Wheel of Names (no student accounts, nickname and code, no ads, school or teacher provides consent, first names only), India's DPDP Act and Rules 2025 (children are under 18; verifiable parental consent and a ban on tracking and behavioural monitoring of children, enforceable from May 2027; educational institutions have a limited exemption), the amended COPPA Rule (compliance from April 2026), and the UK Children's Code (high privacy by default, no nudging). Changes that came out of it: equal-weight Accept and Decline, first-names advice on the teachers FAQ and in the policy, and no analytics even for a teacher's creation of a Classroom room. A `/code-review high` pass also led to the Classroom host fix, a deep Sentry scrub (spans, query-string filters, channel names), cross-tab consent, signing out the anonymous session on Delete my data, and exact retention wording. A second pass closed the remaining ways a room code could reach a third party: the referrer after leaving a room (now always origin plus path, room pages reduced to `/room`), in-app navigation into a room before React re-renders (the history methods are wrapped so GA is disabled before the address changes, instead of relying on a GA dashboard setting), and Sentry log messages and channel names (`for room X`, `city:X`, `room_scores_X`, `code=X`; patterns now match only the real code alphabet, so file paths like `app/room/page.js` are left intact). Also: page views counted once per path, `created_by` for existing rooms set to the current host (a third review showed that guessing from the earliest participant could hand a rejoining teacher's room to a student, and using the host keeps the app and the server agreeing on who the teacher is), Back/Forward navigation into a room also disables GA first, the Sentry "room X" pattern made case-sensitive for the code, and sign-out awaited before Delete my data navigates away.
