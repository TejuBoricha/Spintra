...
5. `package.json`'s `"ci"` script still ran `npm audit --audit-level=high`, out of sync with `ci.yml`'s newly-tightened `--audit-level=moderate` — breaking the documented "`npm run ci` mirrors CI's `validate` job exactly" invariant (`docs/TASKS.md`, `docs/ARCHITECTURE.md`). A future moderate-only advisory would have failed the real GitHub job while `npm run ci` stayed falsely clean. Tightened to match.
...
**Files Modified:** `package.json` (`next` 16.2.10→16.3.5 exact pin, `eslint-config-next` 16.2.10→16.3.5, `"ci"` script's audit-level `high`→`moderate`), `package-lock.json` (dependency tree updated accordingly), `.github/workflows/ci.yml` (audit threshold `high`→`moderate`, comment rewritten), `scripts/check-docs-drift.mjs` (new Check 7b: npm audit threshold sync between `ci.yml` and package.json's `"ci"` script), `docs/TASKS.md` (new Low Priority item for the surfaced lint warning; stale npm-audit references corrected), `docs/HANDOFF.md` (new top "Last Completed Task" entry covering this session, old one relabeled "Prior state" per the file's existing convention; "Current Blockers"/"Next Steps" updated), `docs/AI_CONTEXT.md` ("Current Focus" rewritten), `docs/SPINTRA_CITY_SPEC.md` (§12 item 3 marked done, with the not-yet-pushed caveat).
