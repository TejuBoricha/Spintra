...
## ADR-011: Adopt Ponytail's minimal-code decision ladder into AI_RULES.md
- **Status:** Approved & Implemented
- **Date:** 2026-09-18
- **Context:** User asked to "implement" the [Ponytail](https://github.com/dietrichgebert/ponytail) GitHub repo in this project. Ponytail isn't a code library — it's a prompt-engineering ruleset for AI coding agents (its own `AGENTS.md` + Claude Code skills/hooks) that enforces a "does this need to exist → already in the codebase → stdlib → native feature → installed dependency → one line → only then write code" decision order before generating code. This repo already runs its own `AGENTS.md`/`docs/AI_RULES.md`-based engineering governance system (PIIA, Mandatory Change Reports, doc-sync workflow) — adopting Ponytail's own `AGENTS.md` wholesale would collide with that, not extend it.
- **Decision:** Merge Ponytail's decision ladder and its supporting rules (root-cause-over-symptom bug fixing, no unrequested abstractions/dependencies/boilerplate, shortest-diff-once-understood, mark deliberate simplifications with a ceiling comment, exceptions for trust-boundary validation/error-handling/security/accessibility/explicit requests) into `docs/AI_RULES.md` §2 (Core Development Principles) as a new subsection, leaving `AGENTS.md` and the rest of the workflow untouched.
- **Alternatives Considered:**
  - *Install as a Claude Code plugin* (skills + hooks via Ponytail's own plugin mechanism) — rejected by the user in favor of merging principles into the existing constitution, so the guidance applies uniformly regardless of which AI tool is used, not just Claude Code.
  - *Vendor the repo's files in as-is* (its own `AGENTS.md`, `skills/`, rule dirs) — rejected: Ponytail's `AGENTS.md` would directly conflict with this repo's own bootstrap file and PIIA/reporting workflow.
- **Consequences:** No behavior change to the app; this only changes what future AI sessions are expected to do before writing code. One adaptation from the source: Ponytail's literal `ponytail:` comment-tag convention was dropped in favor of a plain comment requirement, since this repo doesn't run the plugin and importing the literal tag would be meaningless here.
- **Follow-up Actions:** None — this is a self-contained documentation change.

---

## ADR-012: Trial [Graphify](https://github.com/Graphify-Labs/graphify) locally, hold off on workflow integration
- **Status:** Trial only — not adopted into the team workflow
- **Date:** 2026-09-18
- **Context:** User asked to check feasibility of integrating Graphify (a `tree-sitter`-based tool that AST-parses a codebase into a queryable knowledge graph — nodes/edges/communities — with optional LLM-based semantic extraction for docs/PDFs/media). Toolchain (Python 3.13, `uv`) was already present. Installed via `uv tool install graphifyy` (v0.9.63, released 2 days prior — pre-1.0) and ran `graphify extract . --code-only` (local AST only, no API key, nothing left the machine) rather than `graphify install`/`claude install`, which would have written into `CLAUDE.md` and registered a PreToolUse hook — a workflow change, not a trial.
- **Decision:** Keep it as a local, gitignored exploration tool for now. Verified against ground truth already known from this session (`god-nodes` surfaced `skipIfDemoMode()`, the exact helper from `HANDOFF.md`'s CI-fix entry; `explain`/`query` correctly traced real call sites) — the structural graph is accurate. Not installing the CLAUDE.md hook, not committing `graphify-out/`, not running any LLM-backed extraction yet.
- **Alternatives Considered:**
  - *Full workflow integration now* (`claude install` + git hooks + committed `graphify-out/`) — deferred: two real gaps found during the trial make this premature (see Consequences).
  - *Reject outright* — not chosen: the tool demonstrably produced correct, useful answers about this exact codebase in the trial; the gaps are closable, not disqualifying.
- **Consequences:** Two gaps found empirically, not assumed from the README: (1) the 98 SQL migrations — the actual DB schema/RPC/RLS layer, where most of Spintra City's game logic lives — aren't parsed without the separate `graphifyy[sql]` extra (`tree_sitter_sql`); today's graph is TS/JS-app-layer only. (2) Community labels stay generic `Community N` placeholders without an LLM pass (`graphify label`), so the human-navigation layer isn't usable in the fully-local, no-API-key mode that was deliberately chosen for this trial. `graphify-out/` (~3MB) is gitignored (`.gitignore`), regenerable via `graphify extract . --code-only`, not part of the tracked docs system.
- **Follow-up Actions:** None queued — revisit only if/when there's a concrete reason to close the SQL gap or commit to team-wide adoption (`claude install`, git hooks, committed graph). Not a blocker on anything else in `TASKS.md`.
