...
- **Game state replay**: every activity's live state is a capped, ordered event log (max 200 events) kept in `room_activity_state` (`{type, seq, events}`). *Updated by migration 0106 (2026-09-26):* only the server writes it. `send_room_event` numbers and records each event before broadcasting it; clients can no longer write the table. (Before 0106, any client persisted its own copy on a 600ms/2s debounce, which is what made the forgery findings below possible.)
- **Game actions** (`sendActivityEvent`) *before migration 0106* were broadcast over the private `room:{code}` channel authorized by `is_member_of_room()` (migration 0036): membership, not host status, with no host check in the transport and "host-only" actions enforced only by the UI. *Since 0106* they go through `send_room_event`, which accepts host events from the host only and stamps the real sender on player events, and arrive on `room:{code}:events`, which members can't send to.
...
- **Superseded by migration 0106 (2026-09-26, audit G-1/D-8):** the tournament-only trigger checked only the NEW state, so a non-host could replace a running tournament with another game. Clients no longer write `room_activity_state` at all: every game event goes through `send_room_event`, which accepts host events from the host only, stamps the real sender on player events, and records the log itself.
