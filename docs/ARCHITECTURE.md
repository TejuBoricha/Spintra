...
| 0057 | `guess_number_get_secret` | Modifies guess number logic to allow the host to view the secret. |
| 0058 | `fix_host_trigger_regression` | Fixes a regression where host trigger changes blocked certain actions. |
| 0059 | `tournament_fixes` | Increases `room_activity_state_size_check` to 500KB to safely accommodate maximum-capacity tournament states. |

**Current status:** all 59 applied; RLS enabled on all 13 `public` schema tables (`rooms`, `room_participants`, `chat_messages`, `activity_prompts`, `trivia_questions`, `room_bans`, `message_reports`, `guess_number_secrets`, `guess_number_attempts`, `room_activity_state`, `analytics_events`, `room_scores`, `moderation_actions`) plus `realtime.messages`; latest migration is `0059_tournament_fixes`. Note: 0008 and 0010 were re-applied in Session 37, and 0009 in Session 40, after discovering their tracked "applied" status didn't match reality (see `CHANGELOG_AI.md` Session 37/40) — the migration numbering itself didn't change, only their actual execution against the live database.
...
5. ~~Decide on and enable GitHub branch protection for `main`~~ — **done, Session 42.** Confirmed via the GitHub API that `main` had zero protection (404 "Branch not protected"), then enabled it: `validate` and `db-integration` must both pass before merging (`strict: true`, so the branch must be up to date with `main` first), force-pushes and branch deletion blocked. No PR-review requirement and `enforce_admins: false` — deliberately low-friction for a solo repo pre-launch, not the strictest possible policy. No CD step exists yet, and correctly doesn't: there's no deploy target chosen (see checklist item 1 above), so there's nothing to continuously deploy *to*. Add a CD job once a host is picked.
