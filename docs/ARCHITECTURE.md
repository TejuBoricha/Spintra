...
**Current status:** all 32 applied; RLS enabled on all 8 tables; latest policy is `0024_fix_participants_update_recursion`; latest migration is `0032_moderation_event_observability`. Note: 0008 and 0010 were re-applied in Session 37, and 0009 in Session 40, after discovering their tracked "applied" status didn't match reality (see `CHANGELOG_AI.md` Session 37/40) — the migration numbering itself didn't change, only their actual execution against the live database.
