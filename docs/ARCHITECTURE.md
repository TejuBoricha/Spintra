...
**Current status:** all 26 applied; RLS enabled on all 7 tables; latest policy is `0024_fix_participants_update_recursion`; latest migration is `0026_fix_capacity_check_online_only`. Note: 0008 and 0010 were re-applied in Session 37, and 0009 in Session 40, after discovering their tracked "applied" status didn't match reality (see `CHANGELOG_AI.md` Session 37/40) — the migration numbering itself didn't change, only their actual execution against the live database.
